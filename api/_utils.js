...
async function audit(db, action, data={}){
  ...
  await safeAdd(db.collection('historico_auditoria'), {
    ...
    criadoEm: now,
    versao: '1.6.0-rc2.5.1-fardamento-variacoes'
  });
}
...
async function reserveUniformOrder(db, body, actor, student, customer){
  ...
  return {pedidoId:orderRef.id,totalCentavos:total,checkoutTotal:required,saldoAtual,useBalance,creditUsed,debtRequired,model,size,gender:normalizedGender,qty,unit,estoqueDisponivel:available};
}
...
function addUniformStockRequirement(target,modelId,size,gender,qty,name='Camisa de farda'){
  const normalizedSize=String(size||'').trim().toUpperCase(),adult=['P','M','G','GG','XGG'].includes(normalizedSize),normalizedGender=adult?String(gender||'').trim().toLowerCase():'';
  const stockId=uniformStockId(modelId||'camisa_padrao',normalizedSize,normalizedGender),current=target.get(stockId)||{stockId,modeloFardaId:modelId||'camisa_padrao',tamanho:normalizedSize,modelo:normalizedGender,nome,quantidade:0};
  current.quantidade+=Math.max(1,cents(qty||1));target.set(stockId,current);return current;
}
async function validateUniformStockNow(db,line){
  const snap=await db.collection('estoques').doc(line.stockId).get(),doc=snap.exists?snap.data():{},available=doc.configurado?Math.max(0,cents(doc.quantidadeFisica)-cents(doc.quantidadeReservada)):0;
  if(!doc.configurado||available<cents(line.quantidade))throw Object.assign(new Error(`Estoque insuficiente para ${line.nome} · ${line.tamanho}${line.modelo?` · ${line.modelo}`:''}. Disponível: ${available}.`),{status:409});
  return available;
}
function uniformStockReserveUpdate(doc,line,now){return {quantidadeReservada:cents(doc.quantidadeReservada)+cents(line.quantidade),atualizadoEm:now}}
async function normalizeSecretarySaleItems(db,raw=[],channel='presencial'){
  ...
  for(const rawItem of raw){
    const item={...rawItem};
    if(item.tipo==='farda'&&!item.catalogoItemId){const qty=Math.max(1,cents(item.quantidade||1)),unit=cents(item.precoUnitarioCentavos),line=unit*qty,size=String(item.tamanho||item.metadados?.tamanho||'').trim().toUpperCase(),adult=['P','M','G','GG','XGG'].includes(size),gender=adult?String(item.modelo||item.metadados?.modelo||'').trim().toLowerCase():'',modelId=String(item.modeloFardaId||'camisa_padrao');if(unit<=0)throw Object.assign(new Error('Preço de farda inválido.'),{status:400});if(!size)throw Object.assign(new Error('Selecione o tamanho da farda.'),{status:400});if(adult&&!['masculino','feminino'].includes(gender))throw Object.assign(new Error('Selecione Masculino ou Feminino (Baby Look) para tamanhos adultos.'),{status:400});const req=addUniformStockRequirement(uniformRequirements,modelId,size,gender,qty,item.nome||'Camisa de farda');await validateUniformStockNow(db,req);total+=line;lines.push({...item,tipo:'farda',modeloFardaId:modelId,tamanho:size,modelo:gender,quantidade:qty,precoUnitarioCentavos:unit,totalCentavos:line,nome:item.nome||'Camisa de farda',statusAtendimento:'reservado_estoque'});continue;}
    if(item.tipo==='catalogo_item'||item.catalogoItemId){
      ...
      if(cat.tipoOperacional==='fardamento'){
        const modelId=cat.legacyUniformModelId||item.modeloFardaId||'camisa_padrao',adult=adultSize,uniformModel=adult?String(meta.modelo||'').toLowerCase():'';
        if(adult&&!['masculino','feminino'].includes(uniformModel))throw Object.assign(new Error('Selecione Masculino ou Feminino (Baby Look) para tamanhos adultos.'),{status:400});
        const req=addUniformStockRequirement(uniformRequirements,modelId,meta.tamanho,uniformModel,qty,cat.nome);await validateUniformStockNow(db,req);
        lines.push({...common,tipo:'farda',modeloFardaId:modelId,tamanho:meta.tamanho,modelo:uniformModel,statusAtendimento:'reservado_estoque'});
      }else lines.push({...common,tipo:'catalogo_item'});
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});
...
async function buildCheckoutOperation(db, body){
  ...
  if(tipo==='venda_online_secretaria'){
    ...
    const cfg=await getConfig(db),normalized=await normalizeSecretaryMixedSale(db,raw,'online',aluno,cfg),total=normalized.total,lines=normalized.lines,expanded=normalized.expanded,catalogStock=normalized.catalogStock,uniformStock=normalized.uniformStock||[],programacoes=normalized.programacoes;
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await safeSet(ref,{id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:false});
  return evidence;
}
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  const payload=checkout.payloadOperacional||{},lines=Array.isArray(payload.itens)?payload.itens:[],components=Array.isArray(payload.componentes)?payload.componentes:[],catalogStock=Array.isArray(payload.estoquesCatalogo)?payload.estoquesCatalogo:[],uniformStock=Array.isArray(payload.estoquesFarda)?payload.estoquesFarda:[],programacoes=Array.isArray(payload.programacoes)?payload.programacoes:[],total=cents(checkout.pedidoTotalCentavos||payload.pedidoTotalCentavos),external=cents(checkout.totalCentavos),accRef=db.collection('contas_alunos').doc(checkout.alunoId),saleRef=db.collection('vendas').doc(`ONLINE-${safeFinancialId(checkout.orderNsu||checkout.id)}`),linkRef=checkout.vendaOnlineId?db.collection('vendas_online_links').doc(checkout.vendaOnlineId):null;
  ...
  if(!reviewReason)uniformStockRefs.forEach((entry,i)=>{const doc=uniformStockSnaps[i].exists?uniformStockSnaps[i].data():{},available=doc.configurado?Math.max(0,cents(doc.quantidadeFisica)-cents(doc.quantidadeReservada)):0;if(!doc.configurado||available<entry.line.quantidade)reviewReason=`estoque_farda_indisponivel:${entry.line.stockId}`;});
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});
  ...
  uniformStockRefs.forEach((entry,i)=>{const doc=uniformStockSnaps[i].exists?uniformStockSnaps[i].data():{};tx.set(entry.ref,uniformStockReserveUpdate(doc,entry.line,now),{merge:true});});
  ...
  programacoes.forEach((program,i)=>{const orderRef=orderRefs[i];tx.set(orderRef,{id:orderRef.id,vendaId:saleRef.id,orderNsu:checkout.orderNsu||checkout.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,turma:checkout.turma||null,turno:checkout.turno||null,matricula:checkout.matricula||null,tipoPedido:'cantina',modalidade:program.modalidade,dias:program.days,totalCentavos:program.totalCentavos,usarSaldo:payload.usarSaldo===true,statusPagamento:'pago',statusPedido:'confirmado',statusAplicacao:'aplicado',origem:'secretaria_online_venda_mista',criadoPorId:checkout.criadoPorId||null,criadoPorNome:checkout.criadoPorNome||'Secretaria',criadoEm:now,confirmadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});(program.days||[]).forEach((day,j)=>{const occurrenceRef=occurrenceRefs[i][j];tx.set(occurrenceRef,{id:occurrenceRef.id,pedidoId:orderRef.id,vendaId:saleRef.id,orderNsu:checkout.orderNsu||checkout.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,turma:checkout.turma||null,turno:checkout.turno||day.turno||null,matricula:checkout.matricula||null,dataChave:day.dataChave,itens:day.itens||[],componentes:day.componentes||[],quantidadeSalgados:cents(day.quantidadeSalgados),valorCentavos:cents(day.totalCentavos),status:'pendente_entrega',statusPagamento:'pago',origem:'secretaria_online_venda_mista',criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});});});
  let idx=0;for(const line of lines.filter(x=>x.tipo==='farda')){const fr=db.collection('pedidos_farda').doc(`${saleRef.id}__F${++idx}`);tx.set(fr,{id:fr.id,vendaId:saleRef.id,orderNsu:checkout.orderNsu||checkout.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,turma:checkout.turma||null,produto:line.nome,tamanho:line.tamanho||'',modelo:line.modelo||'',modeloFardaId:line.modeloFardaId||'catalogo',catalogoItemId:line.catalogoItemId||null,quantidade:line.quantidade,precoUnitarioCentavos:line.precoUnitarioCentavos,totalCentavos:line.totalCentavos,statusPagamento:'pago',statusAtendimento:line.statusAtendimento||'aguardando_producao',criadoPor:checkout.criadoPorId||'secretaria_online',criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});}
  ...
  return{status:'confirmado',vendaId:saleRef.id,pedidosCantinaIds:orderRefs.map(r=>r.id),quantidadeEntregas:programacoes.reduce((n,p)=>n+(p.days||[]).length,0),saldoDepoisCentavos:after};
}
...
async function applyCheckoutConfirmation(db, orderNsu, paymentData={}, options={}){
  ...
  if(operationResult.cobrancaDescartada){
    ...
    if(operationResult.status==='confirmado'&&cents(operationResult.quantidadeEntregas)>0){await notify(db,{tipo:'pedido_cantina_confirmado',titulo:'Lanches adicionados à agenda',mensagem:`${checkout.alunoNome||'Aluno'}: ${operationResult.quantidadeEntregas} entrega(s) programada(s) em uma venda online da Secretaria.`,prioridade:'normal',alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula,pagamentoId:orderNsu,origem:checkout.tipo==='venda_online_secretaria'?'venda_online_secretaria':'infinitepay',canal:'online',destinatariosPerfis:['cantina','admin','gestao','secretaria'],destinatariosAlunos:[],acaoPrincipal:'abrir_entregas',acaoLabel:'Ver agenda da cantina'});}
...
async function registerInPersonOperation(db,body={}){
  ...
  let operation=String(body.operacao||body.tipoOperacao||'venda_imediata'),raw=Array.isArray(body.itens)?body.itens:[];
  ...
  const normalized=isAccountOnly?{total:0,lines:[],expanded:new Map(),catalogStock:[],uniformStock:[],programacoes:[]}:await normalizeSecretaryMixedSale(db,raw,'presencial',student,cfg),total=normalized.total,lines=normalized.lines,expanded=normalized.expanded,catalogStock=normalized.catalogStock,uniformStock=normalized.uniformStock||[],programacoes=normalized.programacoes||[];
  ...
  await db.runTransaction(async tx=>{
    ...
    uniformStockRefs.forEach((entry,i)=>{const doc=uniformStockSnaps[i].exists?uniformStockSnaps[i].data():{},available=doc.configurado?Math.max(0,cents(doc.quantidadeFisica)-cents(doc.quantidadeReservada)):0;if(!doc.configurado||available<entry.line.quantidade)throw new Error(`Estoque insuficiente para ${entry.line.nome} · ${entry.line.tamanho}${entry.line.modelo?` · ${entry.line.modelo}`:''}. Disponível: ${available}.`);tx.set(entry.ref,uniformStockReserveUpdate(doc,entry.line,now),{merge:true});});
    ...
    if(applied>0)tx.set(paymentMoveRef,{id:paymentMoveRef.id,alunoId:student.id,tipo:'entrada_conta_aluno',subtipo:isAccountOnly?(operation==='regularizar_debito'?'regularizacao_saldo_secretaria':'credito_secretaria'):'pagamento_venda_secretaria',valorCentavos:applied,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,formaPagamento:method,pagamentos:payments,vendaId:isAccountOnly?null:saleRef.id,usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,dataChave:dateKey(),criadoEm:now});
    if(!isAccountOnly){tx.set(purchaseRef,{id:purchaseRef.id,alunoId:student.id,tipo:'compra',subtipo:'venda_secretaria',valorCentavos:-total,valorCompraCentavos:total,saldoAntesCentavos:afterPayment,saldoDepoisCentavos:after,vendaId:saleRef.id,itens:lines,origem:'secretaria',formaPagamento:method,pagamentos:payments,status:'confirmado',dataChave:dateKey(),criadoEm:now});tx.set(saleRef,{id:saleRef.id,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,origem:'secretaria',operadorId:actor.id,operadorNome:actor.nome,formaPagamento:method,valorBrutoCentavos:total,valorRecebidoCentavos:required,valorLiquidoRecebimentoCentavos:netSettlement,taxaMaquininhaCentavos:cardFee,valorSaldoUtilizadoCentavos:creditUsed,itens:lines,pagamentos:payments,pedidosCantinaIds:orderRefs.map(r=>r.id),caixaId:cash?.id||null,sessaoCaixaId:cash?.id||null,periodoResponsabilidadeId:cash?.periodoAtualId||null,dataChave:dateKey(),criadoEm:now,status:'confirmada',schemaVersion:6,versao:'1.6.0-rc2.5.1-fardamento-variacoes'});}
    payments.forEach((p,i)=>{tx.set(payRefs[i],{id:payRefs[i].id,vendaId:isAccountOnly?null:saleRef.id,alunoId:student.id,alunoNome:student.nome,...p,valorBrutoCentavos:p.metodo==='cartao'?p.valorBrutoPassadoCentavos:p.valorAplicadoCentavos,status:'confirmado',origem:'secretaria_presencial',usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,dataChave:dateKey(),criadoEm:now});if(feeRefs[i])tx.set(feeRefs[i],{id:feeRefs[i].id,pagamentoId:payRefs[i].id,vendaId:isAccountOnly?null:saleRef.id,alunoId:student.id,adquirente:p.adquirente,valorBrutoCentavos:p.valorBrutoPassadoCentavos,valorLiquidoCentavos:p.valorLiquidoCentavos,taxaCentavos:p.taxaCentavos,modalidadeCartao:p.modalidadeCartao,parcelas:p.parcelas,nsu:p.nsu||null,dataChave:dateKey(),criadoEm:now,status:'confirmada'});});
    let ui=0;for(const line of lines.filter(x=>x.tipo==='farda')){const fr=uniformRefs[ui++];tx.set(fr,{id:fr.id,vendaId:saleRef.id,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,produto:line.nome,tamanho:line.tamanho||'',modelo:line.modelo||'',modeloFardaId:line.modeloFardaId||'catalogo',catalogoItemId:line.catalogoItemId||null,quantidade:line.quantidade,precoUnitarioCentavos:line.precoUnitarioCentavos,totalCentavos:line.totalCentavos,statusPagamento:'pago',statusAtendimento:line.statusAtendimento||'aguardando_producao',criadoPor:actor.id,criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'});}
    programacoes.forEach((program,i)=>{const orderRef=orderRefs[i];tx.set(orderRef,{id:orderRef.id,vendaId:saleRef.id,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,turno:student.turno||null,matricula:student.matricula||null,tipoPedido:'cantina',modalidade:program.modalidade,dias:program.days,totalCentavos:program.totalCentavos,usarSaldo:useBalance,statusPagamento:'pago',statusPedido:'confirmado',statusAplicacao:'aplicado',origem:'secretaria_presencial_venda_mista',criadoPorId:actor.id,criadoPorNome:actor.nome,criadoEm:now,confirmadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'});(program.days||[]).forEach((day,j)=>{const occurrenceRef=occurrenceRefs[i][j];tx.set(occurrenceRef,{id:occurrenceRef.id,pedidoId:orderRef.id,vendaId:saleRef.id,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,turno:student.turno||day.turno||null,matricula:student.matricula||null,dataChave:day.dataChave,itens:day.itens||[],componentes:day.componentes||[],quantidadeSalgados:cents(day.quantidadeSalgados),valorCentavos:cents(day.totalCentavos),status:'pendente_entrega',statusPagamento:'pago',origem:'secretaria_presencial_venda_mista',criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.5.1-fardamento-variacoes'},{merge:true});});});
    if(cash&&cashInRef){tx.set(cashInRef,{id:cashInRef.id,caixaId:cash.id,sessaoCaixaId:cash.id,periodoResponsabilidadeId:cash.periodoAtualId||null,tipo:'venda',subtipo:'recebimento_venda',vendaId:isAccountOnly?null:saleRef.id,valorCentavos:cashNet,valorRecebidoCentavos:cashTendered,trocoCentavos:cashChange,valorLiquidoCentavos:cashNet,alunoId:student.id,usuarioId:actor.id,usuarioNome:actor.nome,dataChave:dateKey(),criadoEm:now});tx.set(cashSessionRef,{saldoEsperadoAtualCentavos:admin.firestore.FieldValue.increment(cashNet),atualizadoEm:now},{merge:true});if(cashPeriodRef)tx.set(cashPeriodRef,{saldoEsperadoAtualCentavos:admin.firestore.FieldValue.increment(cashNet),atualizadoEm:now},{merge:true});}
  });
  await audit(db,'venda_presencial_aluno',{vendaId:isAccountOnly?null:saleRef.id,alunoId:student.id,alunoNome:student.nome,valorCentavos:isAccountOnly?applied:total,usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,descricaoHumana:isAccountOnly?(operation==='regularizar_debito'?`${actor.nome} regularizou ${brl(applied)} da conta de ${student.nome}.`:`${actor.nome} adicionou ${brl(applied)} à conta de ${student.nome}.`):`${actor.nome} registrou uma venda de ${brl(total)} para ${student.nome}.`});
  if(programacoes.length){const count=programacoes.reduce((n,p)=>n+(p.days||[]).length,0);await notify(db,{tipo:'pedido_cantina_confirmado',titulo:'Lanches adicionados à agenda',mensagem:`${student.nome}: ${count} entrega(s) programada(s) pela Secretaria.`,prioridade:'normal',alunoId:student.id,alunoNome:student.nome,matricula:student.matricula,pedidoId:orderRefs[0]?.id||null,turma:student.turma||null,origem:'secretaria_presencial',canal:'secretaria',criadoPorId:actor.id,criadoPorNome:actor.nome,criadoPorPerfil:actor.perfil,destinatariosPerfis:['cantina','admin','gestao','secretaria'],destinatariosAlunos:[],acaoPrincipal:'abrir_entregas',acaoLabel:'Ver agenda da cantina'});await notify(db,{tipo:'pedido_cantina_confirmado',titulo:'Lanches programados',mensagem:`Pagamento confirmado. ${count} entrega(s) aguardam atendimento da Cantina.`,prioridade:'normal',alunoId:student.id,alunoNome:student.nome,matricula:student.matricula,pedidoId:orderRefs[0]?.id||null,turma:student.turma||null,origem:'secretaria_presencial',canal:'secretaria',criadoPorId:actor.id,criadoPorNome:actor.nome,criadoPorPerfil:actor.perfil,destinatariosPerfis:[],destinatariosAlunos:[student.id],acaoPrincipal:'abrir_pedido',acaoLabel:'Ver pedido'});}
  return{ok:true,vendaId:isAccountOnly?null:saleRef.id,pagamentosIds:payRefs.map(r=>r.id),pedidosCantinaIds:orderRefs.map(r=>r.id),totalCentavos:total,valorRecebidoCentavos:required,valorAplicadoCentavos:applied,valorLiquidoRecebimentoCentavos:netSettlement,taxaMaquininhaCentavos:cardFee,saldoAntesCentavos:before,saldoDepoisCentavos:after};
}
