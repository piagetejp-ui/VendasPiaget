...
async function audit(db, action, data={}){
  ...
  await safeAdd(db.collection('historico_auditoria'), {
    ...
    criadoEm: now,
    versao: '1.6.0-rc2.7-meu-piaget-familias'
  });
}
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7-meu-piaget-familias'},{merge:true});
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await safeSet(ref,{id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.7-meu-piaget-familias'},{merge:false});
  return evidence;
}
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7-meu-piaget-familias'},{merge:true});
...
async function applyCheckoutConfirmation(db, orderNsu, paymentData={}, options={}){
  ...
  if(operationResult.cobrancaDescartada){
    ...
    if(operationResult.status==='confirmado'){for(const id of operationResult.pedidosFardaIds||[]){const fs=await db.collection('pedidos_farda').doc(id).get().catch(()=>null),o=fs&&fs.exists?fs.data():{};await notify(db,{tipo:Number(o.quantidadeAguardandoProducao||0)>0?'farda_enviada_producao':'pedido_farda_confirmado',titulo:Number(o.quantidadeAguardandoProducao||0)>0?'Farda enviada para produção':'Farda reservada',mensagem:`${o.produto||'Camisa de farda'} · ${o.tamanho||'-'}: ${Number(o.quantidadeAguardandoProducao||0)>0?'aguardando produção':'reservada para entrega'}.`,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula,pedidoId:id,origem:'secretaria_online',canal:'online',destinatariosPerfis:['admin','gestao','secretaria'],destinatariosAlunos:[checkout.alunoId],acaoPrincipal:'abrir_pedido',acaoLabel:'Ver pedido'});}for(const id of operationResult.pedidosOperacionaisIds||[]){const os=await db.collection('pedidos_operacionais').doc(id).get().catch(()=>null),o=os&&os.exists?os.data():{};await notify(db,{tipo:'pedido_operacional_confirmado',titulo:'Pedido confirmado',mensagem:`${o.nome||'Item'}: ${String(o.statusAtendimento||'aguardando_atendimento').replaceAll('_',' ')}.`,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula,pedidoId:id,origem:'secretaria_online',canal:'online',destinatariosPerfis:['admin','gestao','secretaria'],destinatariosAlunos:[checkout.alunoId],acaoPrincipal:'abrir_pedido',acaoLabel:'Ver pedido'});}}
...
async function registerInPersonOperation(db,body={}){
  ...
  if(uniformRefs.length){for(let i=0;i<uniformRefs.length;i++){const line=lines.filter(x=>x.tipo==='farda')[i],prod=cents(line?.quantidadeAguardandoProducao)>0;await notify(db,{tipo:prod?'farda_enviada_producao':'pedido_farda_confirmado',titulo:prod?'Farda enviada para produção':'Farda reservada',mensagem:prod?`${student.nome} · ${line.tamanho}${line.modelo?` · ${line.modelo}`:''}: pedido registrado e aguardando produção.`:`${student.nome} · ${line.tamanho}${line.modelo?` · ${line.modelo}`:''}: unidade reservada e aguardando entrega.`,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,pedidoId:uniformRefs[i].id,origem:'secretaria_presencial',canal:'secretaria',destinatariosPerfis:['admin','gestao','secretaria'],destinatariosAlunos:[student.id],acaoPrincipal:'abrir_pedido',acaoLabel:'Ver pedido'});}}
  if(operationalRefs.length){for(let i=0;i<operationalRefs.length;i++){const line=operationalLines[i];await notify(db,{tipo:'pedido_operacional_confirmado',titulo:'Pedido confirmado',mensagem:`${line.nome}: ${String(operationalOrderInitialStatus(line)).replaceAll('_',' ')}.`,alunoId:student.id,alunoNome:student.nome,turma:student.turma||null,pedidoId:operationalRefs[i].id,origem:'secretaria_presencial',canal:'secretaria',destinatariosPerfis:['admin','gestao','secretaria'],destinatariosAlunos:[student.id],acaoPrincipal:'abrir_pedido',acaoLabel:'Ver pedido'});}}
