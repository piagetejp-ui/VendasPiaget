...
async function audit(db, action, data={}){
  ...
  await safeAdd(db.collection('historico_auditoria'), {
    ...
    criadoEm: now,
    versao: '1.6.0-rc2.3-lanches-pagamento-combinado'
  });
}
...
async function findOpenCashSession(db, caixaId='', point='secretaria', shift=''){
  if(caixaId){
    const snap=await db.collection('caixas').doc(String(caixaId)).get();
    if(snap.exists){
      const data=snap.data()||{};
      if(data.status==='aberto'&&(!point||data.ponto===point))return{id:snap.id,...data};
    }
  }
  const snap=await db.collection('caixas').get();
  const today=dateKey();
  const rows=(snap.docs||[]).map(d=>({id:d.id,...d.data()})).filter(x=>x.status==='aberto'&&(!point||x.ponto===point)&&(!x.dataChave||x.dataChave===today)&&(!shift||x.turno===shift));
...
async function normalizeSecretaryMixedSale(db,raw=[],channel='presencial',student=null,cfg=null){
  ...
  const normal=normalRaw.length?await normalizeSecretarySaleItems(db,normalRaw,channel):{total:0,lines:[],expanded:new Map(),catalogStock:[]};
  ...
  return{total,lines,expanded:normal.expanded,catalogStock:normal.catalogStock,programacoes};
}
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.3-lanches-pagamento-combinado'},{merge:true});
...
async function buildCheckoutOperation(db, body){
  ...
  if(tipo==='venda_online_secretaria'){
    ...
    const cfg=await getConfig(db),normalized=await normalizeSecretaryMixedSale(db,raw,'online',aluno,cfg),total=normalized.total,lines=normalized.lines,expanded=normalized.expanded,catalogStock=normalized.catalogStock,programacoes=normalized.programacoes;
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await safeSet(ref,{id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.3-lanches-pagamento-combinado'},{merge:false});
  return evidence;
}
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  const payload=checkout.payloadOperacional||{},lines=Array.isArray(payload.itens)?payload.itens:[],components=Array.isArray(payload.componentes)?payload.componentes:[],catalogStock=Array.isArray(payload.estoquesCatalogo)?payload.estoquesCatalogo:[],programacoes=Array.isArray(payload.programacoes)?payload.programacoes:[],total=cents(checkout.pedidoTotalCentavos||payload.pedidoTotalCentavos),external=cents(checkout.totalCentavos),accRef=db.collection('contas_alunos').doc(checkout.alunoId),saleRef=db.collection('vendas').doc(`ONLINE-${safeFinancialId(checkout.orderNsu||checkout.id)}`),linkRef=checkout.vendaOnlineId?db.collection('vendas_online_links').doc(checkout.vendaOnlineId):null;
  ...
  const readRefs=[accRef,saleRef,...(stockRef?[stockRef]:[]),...(linkRef?[linkRef]:[]),...catalogRefs.map(x=>x.ref),...scheduleStockRefs.map(x=>x.ref)],snaps=await Promise.all(readRefs.map(r=>tx.get(r)));let pos=0;const accSnap=snaps[pos++],saleSnap=snaps[pos++],stockSnap=stockRef?snaps[pos++]:null,linkSnap=linkRef?snaps[pos++]:null,catalogSnaps=catalogRefs.map(()=>snaps[pos++]),scheduleSnaps=scheduleStockRefs.map(()=>snaps[pos++]);
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.3-lanches-pagamento-combinado'},{merge:true});
...
async function registerInPersonOperation(db,body={}){
  ...
  const normalized=isAccountOnly?{total:0,lines:[],expanded:new Map(),catalogStock:[],programacoes:[]}:await normalizeSecretaryMixedSale(db,raw,'presencial',student,cfg),total=normalized.total,lines=normalized.lines,expanded=normalized.expanded,catalogStock=normalized.catalogStock,programacoes=normalized.programacoes||[];
  ...
  let cash=null;if(cashPayments.length){cash=await findOpenCashSession(db,body.caixaId,'secretaria',body.turnoCaixa||body.turno||'');if(!cash)throw Object.assign(new Error('Abra o caixa da secretaria antes de registrar pagamento em dinheiro.'),{status:409});}
  const saleRef=db.collection('vendas').doc(),paymentMoveRef=db.collection('movimentos_conta').doc(),purchaseRef=db.collection('movimentos_conta').doc(),payRefs=payments.map(()=>db.collection('pagamentos').doc()),feeRefs=payments.map(p=>p.metodo==='cartao'&&cents(p.taxaCentavos)>0?db.collection('taxas_pagamento').doc():null),cashInRef=cash?db.collection('movimentos_caixa').doc():null,now=nowIso();
  const salgadoQty=[...expanded.values()].filter(x=>x.produtoId==='salgado').reduce((a,x)=>a+cents(x.quantidade),0),dailyRef=salgadoQty>0?db.collection('disponibilidade_salgados').doc(dateKey()):null,catalogRefs=catalogStock.map(x=>({line:x,ref:db.collection('catalogo_itens').doc(x.itemId)}));
  ...
  await db.runTransaction(async tx=>{
    const readRefs=[accRef,...(dailyRef?[dailyRef]:[]),...catalogRefs.map(x=>x.ref),...scheduleStockRefs.map(x=>x.ref)],snaps=await Promise.all(readRefs.map(r=>tx.get(r)));let pos=0;const fresh=snaps[pos++],freshAcc=fresh.exists?fresh.data():{},freshBefore=accountNet(freshAcc);if(freshBefore!==before)throw new Error('O saldo do aluno mudou. Revise a venda antes de confirmar.');const dailySnap=dailyRef?snaps[pos++]:null,catalogSnaps=catalogRefs.map(()=>snaps[pos++]),scheduleSnaps=scheduleStockRefs.map(()=>snaps[pos++]);
