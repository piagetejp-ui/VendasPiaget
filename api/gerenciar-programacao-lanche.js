...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==='remarcar'){
      ...
      await notify(db,{tipo:'programacao_lanche_remarcada',titulo:'Entrega de lanche alterada',mensagem:`A entrega foi remarcada para ${newDate.split('-').reverse().join('/')}.`,alunoId,pedidoId:result.pedidoId,ocorrenciaId:result.novaOcorrenciaId||occurrenceId,origem:actor.perfil==='responsavel'?'portal_responsavel':actor.perfil,criadoPorId:actor.id,criadoPorNome:actor.nome,criadoPorPerfil:actor.perfil,destinatariosAlunos:[alunoId],destinatariosPerfis:['cantina','secretaria','gestao','admin']});
