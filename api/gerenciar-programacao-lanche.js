...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==='remarcar'){
      ...
      const oldRef=db.collection('ocorrencias_entrega').doc(occurrenceId),targetRef=db.collection('ocorrencias_entrega').doc();let result={};
      await db.runTransaction(async tx=>{
        ...
        const orderRef=db.collection('pedidos').doc(o.pedidoId),oldStockRef=db.collection('disponibilidade_salgados').doc(o.dataChave),newStockRef=db.collection('disponibilidade_salgados').doc(newDate);
        const [orderSnap,oldStockSnap,newStockSnap]=await Promise.all([tx.get(orderRef),tx.get(oldStockRef),tx.get(newStockRef)]),order=orderSnap.exists?orderSnap.data():{},days=order.dias||[];
        if(days.some(day=>day.dataChave===newDate&&day.dataChave!==o.dataChave))throw Object.assign(new Error('Já existe uma entrega desta programação na nova data.'),{status:409});
        const qty=occurrenceSalgados(o,order),oldStock=oldStockSnap.exists?oldStockSnap.data():{},newStock=newStockSnap.exists?newStockSnap.data():{},planned=cents(newStock.quantidadePlanejada||cfg.quantidadePadraoSalgados||30),available=Math.max(0,planned-used(newStock));
        if(qty>available)throw Object.assign(new Error(`Não há salgados suficientes na nova data. Disponíveis: ${available}.`),{status:409});
        const newDays=days.map(day=>day.dataChave===o.dataChave?{...day,dataChave:newDate,remarcadoDe:o.dataChave}:day).sort((a,b)=>a.dataChave.localeCompare(b.dataChave)),now=nowIso();
        ...
        tx.set(oldRef,{status:'remarcado',remarcadoPara:newDate,remarcadoParaOcorrenciaId:targetRef.id,remarcadoEm:now,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:true});
        tx.set(targetRef,{...o,id:targetRef.id,status:'pendente_entrega',dataChave:newDate,remarcadoDe:o.dataChave,remarcadoDeOcorrenciaId:o.id,remarcadoEm:now,finalizadoEm:null,canceladoEm:null,creditadoCentavos:null,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:false});
    ...
    if(action==='cancelar'){
      ...
      await db.runTransaction(async tx=>{
        ...
        tx.set(accRef,{...splitNet(after),bloqueioSaldoSemanal:after<0?Boolean(acc.bloqueioSaldoSemanal):false,bloqueadoPorLimite:false,atualizadoEm:now},{merge:true});tx.set(move,{id:move.id,alunoId,tipo:'credito',subtipo:'credito_cancelamento_programacao',valorCentavos:value,saldoAntesCentavos:before,saldoDepoisCentavos:after,pedidoId:o.pedidoId,ocorrenciaId:o.id,origem:'cancelamento_responsavel',usuarioId:actor.id,usuarioNome:actor.nome,dataChave:o.dataChave,criadoEm:now});tx.set(stockRef,{pedidosConfirmados:Math.max(0,cents(stock.pedidosConfirmados)-qty),atualizadoEm:now},{merge:true});tx.set(ref,{status:'cancelado_responsavel',creditadoCentavos:value,canceladoEm:now,finalizadoEm:now,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:true});tx.set(orderRef,{atualizadoEm:now},{merge:true});result={pedidoId:o.pedidoId,dataChave:o.dataChave,creditoCentavos:value,saldoDepoisCentavos:after};
      });
