  ...
  try{
    ...
    if(action==='remarcar'){
      ...
      await db.runTransaction(async tx=>{
        ...
        const qty=occurrenceSalgados(o,orderSnap.exists?orderSnap.data():{}),oldStock=oldStockSnap.exists?oldStockSnap.data():{},newStock=newStockSnap.exists?newStockSnap.data():{},planned=cents(newStock.quantidadePlanejada||cfg.quantidadePadraoSalgados||30);
        ...
        tx.set(targetRef,{...o,id:targetRef.id,dataChave:newDate,remarcadoDe:o.dataChave,remarcadoEm:now,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:false});tx.delete(oldRef);tx.set(orderRef,{dias:days,atualizadoEm:now},{merge:true});result={pedidoId:o.pedidoId,oldDate:o.dataChave,newDate};
      });
    ...
    if(action==='cancelar'){
      ...
      await db.runTransaction(async tx=>{
        ...
        tx.set(orderRef,{atualizadoEm:now},{merge:true});result={pedidoId:o.pedidoId,dataChave:o.dataChave,creditoCentavos:value,saldoDepoisCentavos:after};
      });
