...
async function audit(db, action, data={}){
  ...
  const entityType=data.entidadeTipo || (data.vendaId?'venda':data.pedidoId?'pedido':(data.responsavelId||data.responsavelFinanceiroId)?'conta_familiar':data.alunoId?'aluno':null);
  const entityId=data.entidadeId || data.vendaId || data.pedidoId || data.responsavelId || data.responsavelFinanceiroId || data.alunoId || data.pagamentoId || data.orderNsu || null;
  const auditRef=await safeAdd(db.collection('historico_auditoria'), {
    ...
    responsavelId: data.responsavelId || data.responsavelFinanceiroId || null,
    ...
    retencao: 'permanente',
    criadoEm: now,
    versao: '1.6.0-rc2.7.20'
  });
  /* Fila leve, sem leitura e sem hard cap: várias alterações da mesma entidade se fundem em um documento.
     O sistema não depende desta fila para concluir operações; ela serve à reconciliação/diagnóstico incremental. */
  if(entityType&&entityId){const key=`${String(entityType).replace(/[^a-zA-Z0-9_-]/g,'_')}__${String(entityId).replace(/[^a-zA-Z0-9_-]/g,'_')}`;await safeSet(db.collection('entidades_pendentes').doc(key),{entidadeTipo:entityType,entidadeId:String(entityId),responsavelId:data.responsavelId||data.responsavelFinanceiroId||null,alunoId:data.alunoId||null,ultimaAcao:action,ultimaAuditoriaId:auditRef?.id||null,ultimaAlteracaoEm:now,processada:false,versao:'1.6.0-rc2.7.20'},{merge:true});}
}
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.20'},{merge:true});
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await ref.set({id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.7.20'},{merge:false});
  if(orderNsu){
    const patch={
      atualizadoEm:now,
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,alunosIds,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.20'},{merge:true});
