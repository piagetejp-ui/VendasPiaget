...
async function cancelInPersonSale(db,body={}){
  ...
  const revPurchaseRef=purchaseReversalAmount>0?db.collection('movimentos_conta').doc():null,revPaymentRef=applied>0?db.collection('movimentos_conta').doc():null,revCashRef=effectiveOriginalCashNet>0?db.collection('movimentos_caixa').doc():null,now=nowIso(),cancelOrigin=refundMode?'cancelamento_reembolso_venda':'cancelamento_venda_presencial',paymentReversalSubtype=refundMode?'estorno_pagamento_reembolsado':'estorno_pagamento_nao_recebido';let alreadyCancelled=false,resultBalance=null;
  await db.runTransaction(async tx=>{
    ...
    for(const x of freshOccurrences)if(x.snap.exists)tx.set(x.meta.ref,{status:'cancelado',statusPagamento:'cancelado',canceladoPorId:actor.id,canceladoPorNome:actor.nome,canceladoEm:now,motivoCancelamento:cancellationReasonLabelV222(reason),atualizadoEm:now,versao:'1.6.0-rc2.7.24'},{merge:true});
