...
async function audit(db, action, data={}){
  ...
  await safeAdd(db.collection('historico_auditoria'), {
    ...
    criadoEm: now,
    versao: '1.6.0-rc2.7.1-vercel-hobby-hotfix'
  });
}
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.1-vercel-hobby-hotfix'},{merge:true});
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await safeSet(ref,{id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.7.1-vercel-hobby-hotfix'},{merge:false});
  return evidence;
}
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.1-vercel-hobby-hotfix'},{merge:true});
