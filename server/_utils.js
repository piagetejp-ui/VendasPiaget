...
function addUniformStockRequirement(target,modelId,size,gender,qty,name='Camisa de farda'){
  const normalizedSize=String(size||'').trim().toUpperCase(),adult=['P','M','G','GG','XGG'].includes(normalizedSize),normalizedGender=adult?String(gender||'').trim().toLowerCase():'',stockId=uniformStockId(modelId||'camisa_padrao',normalizedSize,normalizedGender),current=target.get(stockId)||{stockId,modeloFardaId:modelId||'camisa_padrao',tamanho:normalizedSize,modelo:normalizedGender,nome:name,quantidade:0,permitirProducao:true};current.quantidade+=Math.max(1,cents(qty||1));target.set(stockId,current);return current;
}
...
async function applyAccountPaymentTx(tx, db, checkout, paymentData={}, openPendencies=[]){
  const accRef=await accountRefForStudent(db,checkout.alunoId);
  const pendencyRefs=openPendencies.map(d=>d.ref);
  const [snap,...pendencySnaps]=await Promise.all([tx.get(accRef),...pendencyRefs.map(r=>tx.get(r))]);
  ...
  {
    let remaining=total;
    const ordered=pendencySnaps.filter(s=>s.exists).sort((a,b)=>String(a.data().criadoEm||'').localeCompare(String(b.data().criadoEm||'')));
    for(const s of ordered){
      if(remaining<=0)break;
      const d=s.data()||{};
      if(d.statusPagamento!=='aguardando_pagamento'||String(d.alunoId)!==String(checkout.alunoId))continue;
      const valor=cents(d.totalCentavos);
      if(valor<=0||valor>remaining)break;
      remaining-=valor;
      settlements.push({ref:s.ref,data:d});
    }
  }
  tx.set(accRef, {
    ...split,
  ...
  tx.set(movRef, {
    ...
    pendenciasQuitadasIds: settlements.map(x=>x.ref.id)
  });
  const settleNow=nowIso();
  for(const {ref,data} of settlements){
    tx.set(ref,{statusPagamento:'pago',pagoViaOrderNsu:checkout.orderNsu||checkout.id,pagoEm:settleNow,atualizadoEm:settleNow},{merge:true});
    if(data.movimentacaoId)tx.set(db.collection('movimentos_conta').doc(String(data.movimentacaoId)),{statusPagamento:'pago',pagoEm:settleNow,atualizadoEm:settleNow},{merge:true});
    if(data.pedidoId){
      const orderCollection=data.categoria==='fardamento'?'pedidos_farda':'pedidos_operacionais';
      tx.set(db.collection(orderCollection).doc(String(data.pedidoId)),{statusPagamento:'pago',pagoEm:settleNow,atualizadoEm:settleNow},{merge:true});
    }
  }
  return {saldoAntesCentavos:oldNet,saldoDepoisCentavos:newNet,pendenciasQuitadasIds:settlements.map(x=>x.ref.id)};
}
...
async function applyCheckoutConfirmation(db, orderNsu, paymentData={}, options={}){
  ...
  const openPendencies=(checkout.tipo==='entrada_conta_aluno'&&checkout.alunoId)
    ? (await db.collection('lancamentos_manuais').where('alunoId','==',checkout.alunoId).where('statusPagamento','==','aguardando_pagamento').limit(50).get()).docs
    : [];
  ...
  try{
    await db.runTransaction(async tx=>{
      ...
      if(discarded){
        ...
        if(c.tipo==='entrada_conta_aluno')operationResult=await applyAccountPaymentTx(tx,db,c,paymentData,openPendencies);
...
async function registerInPersonOperation(db,body={}){
  ...
  const operationId=String(body.operationId||'').trim().replace(/[^A-Za-z0-9_-]/g,'').slice(0,120)||null,idempotencyRef=operationId?db.collection('operacoes_presenciais_ids').doc(operationId):null;
  ...
  await db.runTransaction(async tx=>{
    if(idempotencyRef){const idemSnap=await tx.get(idempotencyRef);if(idemSnap.exists){duplicateOperation=true;return;}}
    ...
    if(idempotencyRef)tx.set(idempotencyRef,{id:operationId,criadoEm:now,alunoId:student.id,operacao:operation,vendaId:isAccountOnly?null:saleRef.id});
  });
  if(duplicateOperation)return{ok:true,duplicado:true,vendaId:isAccountOnly?null:saleRef.id,pagamentosIds:[],pedidosCantinaIds:[],pedidosFardaIds:[],pedidosOperacionaisIds:[],totalCentavos:total,valorRecebidoCentavos:required,valorAplicadoCentavos:0,valorLiquidoRecebimentoCentavos:0,taxaMaquininhaCentavos:0,saldoAntesCentavos:before,saldoDepoisCentavos:before};
