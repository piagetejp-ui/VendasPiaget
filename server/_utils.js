...
async function audit(db, action, data={}){
  ...
  const auditRef=await safeAdd(db.collection('historico_auditoria'), {
    acao:action,acaoTecnica:action,dados:data,tituloHumano:title,descricaoHumana:data.descricaoHumana||data.descricao||'',categoria:category,severidade:data.severidade||'info',icone:data.icone||(action.startsWith('pedido_cantina')?'🍽️':action.startsWith('pedido_farda')?'👕':action==='venda_presencial_aluno'?'🧾':action==='venda_presencial_cancelada'?'↩️':'💳'),
    ...
    entidadeTipo:primary?.tipo||null,entidadeId:primary?.id||null,entidades:entities,buscaNormalizada:auditSearchV221(action,data,title,category,actor,entities),retencao:'permanente',criadoEm:now,versao:'1.6.0-rc2.7.23'
  });
...
async function findOpenCashSession(db, caixaId='', point='secretaria', shift='', actorId='', responsibilityId=''){
  ...
  if(!rows.length){
    // RC2.7.23: consulta somente sessões abertas, mas sem teto oculto de 20 registros.
...
async function replaceActiveCheckoutsForOperation(db,body,operationKey){
  ...
  // RC2.7.23: não limitar a 30 checkouts arbitrários. Percorremos todo o histórico SOMENTE deste aluno,
  ...
  for(const row of rows){
    await row.ref.set({statusAnterior:row.status,status:'substituido_nova_cobranca',statusAplicacao:'substituido_sem_pagamento',checkoutUrlAtivo:false,ocultoParaResponsavel:true,substituidoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.23'},{merge:true});
...
async function recordInfinitePayEvent(db,orderNsu,paymentData={},source='confirmacao'){
  ...
  await ref.set({id:ref.id,orderNsu,source,payload:paymentData,evidence,criadoEm:now,versao:'1.6.0-rc2.7.23'},{merge:false});
  if(orderNsu){
    const patch={
      atualizadoEm:now,
...
async function applyOnlineSecretarySaleTx(tx,db,checkout,paymentData={}){
  ...
  if(reviewReason){
    ...
    tx.set(saleRef,{id:saleRef.id,alunoId:checkout.alunoId,alunoNome:checkout.alunoNome,matricula:checkout.matricula||null,alunosIds,origem:'secretaria_online',orderNsu:checkout.orderNsu||checkout.id,valorBrutoCentavos:total,valorPagoExternoCentavos:external,itens:lines,status:'revisao',motivoRevisao:reviewReason,saldoAntesCentavos:before,saldoDepoisCentavos:afterPayment,dataChave:dateKey(),criadoEm:now,atualizadoEm:now,versao:'1.6.0-rc2.7.23'},{merge:true});
...
function cancellationBlockersV222({uniformOrders=[],operationalOrders=[],occurrences=[]}={}){
  const blockers=[];
  ...
  for(const o of occurrences){const st=String(o.status||'');if(!['pendente_entrega','programado','cancelado',''].includes(st))blockers.push(`Entrega de ${o.dataChave||o.id}: já está em “${st.replaceAll('_',' ')}”.`)}
  return blockers;
}
...
async function cancelInPersonSale(db,body={}){
  ...
  const refundMode=String(body.tipoOperacao||'')==='reembolso_recebido',refundMethod=String(body.formaReembolso||'').toLowerCase(),refundReference=String(body.referenciaReembolso||'').trim();
  ...
  const [student,payments,fees,cashMoves,uniformOrders,operationalOrders,cantinaOrders,occurrences,impact]=await Promise.all([
    getStudent(db,sale.alunoId),querySaleRelatedV222(db,'pagamentos',saleId),querySaleRelatedV222(db,'taxas_pagamento',saleId),querySaleRelatedV222(db,'movimentos_caixa',saleId),querySaleRelatedV222(db,'pedidos_farda',saleId),querySaleRelatedV222(db,'pedidos_operacionais',saleId),querySaleRelatedV222(db,'pedidos',saleId),querySaleRelatedV222(db,'ocorrencias_entrega',saleId),rebuildSaleStockImpactV222(db,sale)
  ]);if(!student)throw Object.assign(new Error('Aluno da venda não encontrado.'),{status:409});
  const blockers=cancellationBlockersV222({uniformOrders,operationalOrders,occurrences});if(blockers.length)throw Object.assign(new Error('A venda já possui atendimento/entrega posterior e não pode ser cancelada automaticamente.'),{status:409,details:{blockers}});
  const accRef=await accountRefForStudent(db,sale.alunoId,student),total=Math.max(0,cents(sale.valorBrutoCentavos)),applied=Math.max(0,cents((sale.pagamentos||[]).reduce((n,p)=>n+cents(p.valorAplicadoCentavos),0)||sale.valorRecebidoCentavos||sale.valorPagoExternoCentavos)),catalogImpact=impact.catalogStock||[],uniformImpact=mergeUniformReversalV222(uniformOrders),scheduleImpact=mergeScheduleReversalV222(occurrences),cashOriginal=cashMoves.filter(m=>String(m.tipo||'')==='venda'&&String(m.status||'')!=='cancelado_sem_recebimento'),cashNet=Math.max(0,cashOriginal.reduce((n,m)=>n+cents(m.valorLiquidoCentavos||m.valorCentavos),0));
  ...
  const revPurchaseRef=db.collection('movimentos_conta').doc(),revPaymentRef=applied>0?db.collection('movimentos_conta').doc():null,revCashRef=effectiveOriginalCashNet>0?db.collection('movimentos_caixa').doc():null,now=nowIso();let alreadyCancelled=false,resultBalance=null;
  await db.runTransaction(async tx=>{
    ...
    const snaps=await Promise.all(refs.map(r=>tx.get(r)));let pos=0;const accSnap=snaps[pos++],acc=accSnap.exists?accSnap.data()||{}:{};const current=accountNet(acc),afterPurchaseReversal=current+total,afterFinal=afterPurchaseReversal-applied;resultBalance={before:current,afterPurchaseReversal,after:afterFinal};
    ...
    const freshBlockers=cancellationBlockersV222({uniformOrders:freshUniform.filter(x=>x.snap.exists).map(x=>({id:x.meta.id,...x.snap.data()})),operationalOrders:freshOperational.filter(x=>x.snap.exists).map(x=>({id:x.meta.id,...x.snap.data()})),occurrences:freshOccurrences.filter(x=>x.snap.exists).map(x=>({id:x.meta.id,...x.snap.data()}))});if(freshBlockers.length)throw Object.assign(new Error('A venda recebeu atendimento enquanto o cancelamento era preparado.'),{status:409,details:{blockers:freshBlockers}});
    ...
    tx.set(revPurchaseRef,{id:revPurchaseRef.id,alunoId:sale.alunoId,alunosIds:sale.alunosIds||[sale.alunoId],tipo:'estorno',subtipo:'estorno_compra_venda_cancelada',valorCentavos:total,saldoAntesCentavos:current,saldoDepoisCentavos:afterPurchaseReversal,vendaId:saleId,vendaOriginalId:saleId,itens:sale.itens||[],origem:'cancelamento_venda_presencial',formaPagamento:sale.formaPagamento||null,status:'confirmado',motivoCodigo:reason,motivo:cancellationReasonLabelV222(reason),usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,dataChave:dateKey(),criadoEm:now,versao:'1.6.0-rc2.7.23'});
    if(revPaymentRef)tx.set(revPaymentRef,{id:revPaymentRef.id,alunoId:sale.alunoId,alunosIds:sale.alunosIds||[sale.alunoId],tipo:'estorno',subtipo:'estorno_pagamento_nao_recebido',valorCentavos:-applied,saldoAntesCentavos:afterPurchaseReversal,saldoDepoisCentavos:afterFinal,vendaId:saleId,vendaOriginalId:saleId,origem:'cancelamento_venda_presencial',formaPagamento:sale.formaPagamento||null,status:'confirmado',motivoCodigo:reason,motivo:cancellationReasonLabelV222(reason),usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,dataChave:dateKey(),criadoEm:now,versao:'1.6.0-rc2.7.23'});
    ...
    for(const x of freshOccurrences)if(x.snap.exists)tx.set(x.meta.ref,{status:'cancelado',statusPagamento:'cancelado',canceladoPorId:actor.id,canceladoPorNome:actor.nome,canceladoEm:now,motivoCancelamento:cancellationReasonLabelV222(reason),atualizadoEm:now,versao:'1.6.0-rc2.7.23'},{merge:true});
    ...
    tx.set(saleRef,{status:'cancelada',statusFinanceiro:refundMode?'reembolsado':'estornado_sem_recebimento',valorRecebidoEfetivoCentavos:0,cancelamentoTipo:refundMode?'cancelamento_com_reembolso_ao_pagador':'cancelamento_operacional_sem_reembolso',reembolsoId:refundRef?.id||null,formaReembolso:refundMode?refundMethod:null,referenciaReembolso:refundMode?(refundReference||null):null,motivoCancelamentoCodigo:reason,motivoCancelamento:cancellationReasonLabelV222(reason),detalhesCancelamento:detail||null,canceladoPorId:actor.id,canceladoPorNome:actor.nome,canceladoPorPerfil:actor.perfil,canceladoEm:now,atualizadoEm:now,estornoMovimentoCompraId:revPurchaseRef.id,estornoMovimentoPagamentoId:revPaymentRef?.id||null,estornoMovimentoCaixaId:revCashRef?.id||null,impactoEstoqueCancelamentoFonte:impact.source,versaoCancelamento:'1.6.0-rc2.7.23'},{merge:true});
  });
  if(alreadyCancelled)return {ok:true,vendaId:saleId,status:'cancelada',alreadyCancelled:true};
  await audit(db,'venda_presencial_cancelada',{vendaId:saleId,alunoId:sale.alunoId,alunoNome:sale.alunoNome||student.nome,responsavelFinanceiroId:sale.responsavelFinanceiroId||student.responsavelFinanceiroId||accRef.id,valorCentavos:total,valorPagamentoEstornadoCentavos:applied,valorCaixaEstornadoCentavos:effectiveOriginalCashNet,valorReembolsadoCentavos:refundMode?applied:0,formaReembolso:refundMode?refundMethod:null,motivoCodigo:reason,motivo:cancellationReasonLabelV222(reason),detalhes:detail||null,usuarioId:actor.id,usuarioNome:actor.nome,usuarioPerfil:actor.perfil,sessaoCaixaId:cashSessionId||null,severidade:'warning',descricaoHumana:`${actor.nome} cancelou a venda presencial ${saleId} de ${brl(total)} de ${sale.alunoNome||student.nome}. Motivo: ${cancellationReasonLabelV222(reason)}.`});
  return {ok:true,vendaId:saleId,status:'cancelada',alreadyCancelled:false,totalCentavos:total,valorPagamentoEstornadoCentavos:applied,valorCaixaEstornadoCentavos:effectiveOriginalCashNet,valorReembolsadoCentavos:refundMode?applied:0,formaReembolso:refundMode?refundMethod:null,saldoAntesCancelamentoCentavos:resultBalance?.before,saldoDepoisCancelamentoCentavos:resultBalance?.after};
}
