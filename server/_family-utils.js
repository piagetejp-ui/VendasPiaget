...
function familyFirebaseUid(responsavelId){return `familia_${String(responsavelId||'').replace(/[^A-Za-z0-9:_-]/g,'_')}`.slice(0,128)}
async function syncFamilyAuthMirror(db,session){
  if(!session?.responsavelId||!session?.id)throw new Error('Sessão familiar inválida.');
  let expiresAt=String(session.expiraEm||'');
  if(!expiresAt){const snap=await db.collection('sessoes_meu_piaget').doc(String(session.id)).get();if(!snap.exists)throw new Error('Sessão familiar não encontrada.');const row=snap.data()||{};expiresAt=String(row.expiraEm||'');session={...row,...session,id:snap.id,expiraEm:expiresAt}}
  const expiresMs=new Date(expiresAt).getTime();if(!Number.isFinite(expiresMs)||expiresMs<=Date.now())throw new Error('Sessão familiar expirada.');
  const uid=familyFirebaseUid(session.responsavelId),students=await studentsForResponsible(db,session.responsavelId),alunosIds=students.map(a=>String(a.id)).slice(0,12),now=nowIso();
  await db.collection('familias_auth').doc(String(session.id)).set({firebaseUid:uid,responsavelId:String(session.responsavelId),sessionId:String(session.id),alunosIds,ativo:true,expiraEm:expiresAt,expiraEmMs:Math.trunc(expiresMs),atualizadoEm:now},{merge:true});
  return{uid,alunosIds,expiraEm:expiresAt,expiraEmMs:Math.trunc(expiresMs)};
}
async function deactivateFamilyAuthMirror(db,sessionId,motivo='sessao_encerrada'){const id=String(sessionId||'');if(!id)return;await db.collection('familias_auth').doc(id).set({ativo:false,motivoInativacao:String(motivo||'sessao_encerrada'),inativadoEm:nowIso(),atualizadoEm:nowIso()},{merge:true}).catch(()=>{})}
async function revokeFamilySessions(db,responsavelId,{exceptId=''}={}){if(!responsavelId)return 0;const snap=await db.collection('sessoes_meu_piaget').where('responsavelId','==',String(responsavelId)).get().catch(()=>({docs:[]}));const active=snap.docs.filter(d=>d.id!==String(exceptId||'')&&d.data()?.status==='ativa');if(!active.length)return 0;const batch=db.batch(),now=nowIso();for(const d of active){batch.set(d.ref,{status:'encerrada',encerradoEm:now,motivoEncerramento:'seguranca_acesso',atualizadoEm:now},{merge:true});batch.set(db.collection('familias_auth').doc(d.id),{ativo:false,motivoInativacao:'seguranca_acesso',inativadoEm:now,atualizadoEm:now},{merge:true})}await batch.commit();return active.length}
async function familyFirebaseToken(db,session){if(!session?.responsavelId||!session?.id)throw new Error('Sessão familiar inválida.');const mirror=await syncFamilyAuthMirror(db,session);return admin.auth().createCustomToken(mirror.uid,{role:'responsavel',responsavelId:String(session.responsavelId),sessionId:String(session.id),alunosIds:mirror.alunosIds,authSchema:'family-mirror-v1'})}
...
module.exports={normalizeCpf,normalizeMatricula,validCpf,cpfHash,tokenHash,randomToken,passwordOk,makePassword,verifyPassword,findResponsibleByCpf,studentsForResponsible,publicStudent,familyPayload,createFamilySession,validateFamilySession,revokeFamilySessions,familySessionTokenFromReq,familyCookieHeader,familyFirebaseUid,syncFamilyAuthMirror,deactivateFamilyAuthMirror,familyFirebaseToken,verifyFamilyForStudent,verifyFamilyFirebaseForStudent,verifyCheckoutPrincipal,verifyStaff,enforceAccessRateLimit,FAMILY_COOKIE};
