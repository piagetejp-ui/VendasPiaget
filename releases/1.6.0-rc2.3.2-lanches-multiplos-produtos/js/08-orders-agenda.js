...
(()=>{
'use strict';
const V150_VERSION='1.6.0-rc2.3.2-lanches-multiplos-produtos';
...
function nextBusinessV150(from=dateKey()){let key=from;for(let i=0;i<10;i++){const d=dateFromKeyV150(key);if(![0,6].includes(d.getDay())&&!(cfg150().diasSemAula||[]).includes(key))return key;key=addDaysV150(key,1)}return from}
...
function dailyAvailabilityV150(d={}){const planned=Number(d.quantidadePlanejada||cfg150().quantidadePadraoSalgados||30);const reserved=activeReservationsV150(d.reservas);const confirmed=Number(d.pedidosConfirmados||0),cash=Number(d.vendidoDinheiro||0)+Number(d.vendidoAvulso||0),account=Number(d.consumoConta||0);return{planned,reserved,confirmed,cash,account,released:Number(d.liberadoPorAusencia||0),available:Math.max(0,planned-reserved-confirmed-cash-account)}}
...
async function loadOrdersForStudentV150(studentId){
  try{const q=await db.collection('pedidos').where('alunoId','==',studentId).get();return q.docs.map(d=>({id:d.id,...d.data()}));}
  catch(e){const q=await db.collection('pedidos').get().catch(()=>({docs:[]}));return q.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.alunoId===studentId)}
}
...
function plannerDatesV150(){
  ...
  if(mode==='avulso'){const value=$('#v150OrderDate')?.value;return value?[value]:[]}
  if(mode==='semanal'){const start=$('#v150OrderWeek')?.value;if(!start)return[];const arr=[];for(let i=0;i<7;i++){const key=addDaysV150(start,i),d=dateFromKeyV150(key);if(![0,6].includes(d.getDay())&&key>=dateKey()&&!(cfg150().diasSemAula||[]).includes(key))arr.push(key)}return arr}
  const value=$('#v150OrderMonth')?.value;if(!value)return[];const [y,m]=value.split('-').map(Number),arr=[];for(let day=1;day<=new Date(y,m,0).getDate();day++){const d=new Date(y,m-1,day,12),key=dateKey(d);if(![0,6].includes(d.getDay())&&key>=dateKey()&&!(cfg150().diasSemAula||[]).includes(key))arr.push(key)}return arr;
}
...
async function availabilityForDateV1523(day){const cached=availabilityCacheV1523.get(day);if(cached&&Date.now()-cached.at<30000)return cached.info;const snap=await db.collection('disponibilidade_salgados').doc(day).get(),info=dailyAvailabilityV150(snap.exists?snap.data():{});availabilityCacheV1523.set(day,{at:Date.now(),info});return info}
async function runAvailabilityRefreshV1523(){
  const request=++availabilityRequestV1523,rows=plannerRowsV150();
  await Promise.all(rows.map(async x=>{const target=x.row.querySelector('.v150-availability');if(!target)return;if(!x.check?.checked){target.className='v150-availability';target.textContent='Dia não selecionado';return}if(!x.items.length){target.className='v150-availability err';target.textContent='Adicione um produto';return}const q=x.items.reduce((sum,item)=>sum+productSalgadoQtyV150(item.product?.value,Math.min(10,Math.max(1,Number(item.qty?.value||1)))),0);if(!q){target.className='v150-availability ok';target.textContent='Itens disponíveis';return}try{const info=await availabilityForDateV1523(x.date);if(request!==availabilityRequestV1523)return;target.className=`v150-availability ${info.available>=q?'ok':info.available>0?'warn':'err'}`;target.textContent=info.available>=q?`${q} salgado(s) reservado(s) · ${info.available} disponível(is)`:info.available>0?`Precisa de ${q}; só ${info.available} disponível(is)`:'Sem salgados disponíveis';}catch(e){target.className='v150-availability warn';target.textContent='Validação final na reserva';}}));
}
...
function toggleAllOrderDaysV150(value){plannerRowsV150().forEach(x=>x.check.checked=value);refreshOrderAvailabilityV150(true)}
...
async function setOccurrenceV150(id,status,reason=''){if(!['entregue','aluno_ausente','nao_entregue'].includes(status))return;const ref=db.collection('ocorrencias_entrega').doc(id);try{let pedidoId='';await db.runTransaction(async tx=>{const s=await tx.get(ref);if(!s.exists)throw new Error('Pedido não encontrado.');const o={id:s.id,...s.data()};pedidoId=o.pedidoId||'';if(V150_FINAL_STATUSES.includes(o.status))throw new Error('Este pedido já recebeu uma situação final.');const now=nowIso(),actor=state.user||{id:'cantina',nome:'Operador da cantina',perfil:'cantina'};if(status==='entregue'){tx.set(ref,{status:'entregue',entregueEm:now,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:true});return}const accRef=db.collection('contas_alunos').doc(o.alunoId),stockRef=db.collection('disponibilidade_salgados').doc(o.dataChave),accSnap=await tx.get(accRef),stockSnap=await tx.get(stockRef),acc=accSnap.exists?accSnap.data():{},stock=stockSnap.exists?stockSnap.data():{},value=Number(o.valorCentavos||(o.itens||[]).reduce((sum,x)=>sum+Number(x.totalCentavos||x.quantidade*x.precoUnitarioCentavos||0),0)),before=netV150(acc),after=before+value,q=Number(o.quantidadeSalgados||(o.componentes||[]).filter(x=>x.produtoId==='salgado').reduce((sum,x)=>sum+Number(x.quantidade||0),0)),move=db.collection('movimentos_conta').doc();tx.set(accRef,{...splitV150(after),bloqueioSaldoSemanal:after<0?Boolean(acc.bloqueioSaldoSemanal):false,bloqueadoPorLimite:after<0&&Number(acc.limiteFiadoCentavos||0)>0&&Math.abs(after)>=Number(acc.limiteFiadoCentavos||0),atualizadoEm:now},{merge:true});tx.set(move,{id:move.id,alunoId:o.alunoId,tipo:'credito',subtipo:status==='aluno_ausente'?'credito_ausencia':'credito_nao_entrega',valorCentavos:value,saldoAntesCentavos:before,saldoDepoisCentavos:after,pedidoId:o.pedidoId,ocorrenciaId:id,origem:'pedido_pago_nao_entregue',usuarioId:actor.id,usuarioNome:actor.nome,dataChave:o.dataChave,criadoEm:now,motivo:reason||status});tx.set(stockRef,{pedidosConfirmados:Math.max(0,Number(stock.pedidosConfirmados||0)-q),liberadoPorAusencia:Number(stock.liberadoPorAusencia||0)+q,atualizadoEm:now},{merge:true});tx.set(ref,{status,motivoNaoEntrega:reason||(status==='aluno_ausente'?'Aluno ausente':'Não entregue'),creditadoCentavos:value,finalizadoEm:now,alteradoPorId:actor.id,alteradoPorNome:actor.nome,atualizadoEm:now},{merge:true})});await audit('ocorrencia_'+status,{ocorrenciaId:id,pedidoId,usuarioId:state.user?.id,usuarioNome:state.user?.nome,motivo:reason});await syncOrderStatusV150(pedidoId);try{const current=await ref.get(),o=current.exists?current.data():{};await createNotificationV134({tipo:status==='entregue'?'pedido_entregue':'pedido_nao_entregue',titulo:status==='entregue'?'Lanche entregue':'Entrega não realizada',mensagem:status==='entregue'?`A entrega de ${dateLabelV150(o.dataChave)} foi confirmada.`:`A entrega de ${dateLabelV150(o.dataChave)} não foi realizada e o valor retornou para a conta.`,prioridade:'normal',alunoId:o.alunoId,alunoNome:o.alunoNome,pedidoId:o.pedidoId,destinatariosAlunos:[o.alunoId],destinatariosPerfis:['cantina','secretaria','gestao','admin'],acaoPrincipal:'abrir_pedido'})}catch(e){console.warn('notificacao entrega',e)}toast(status==='entregue'?'Entrega confirmada.':'Valor devolvido como crédito e salgado liberado.');renderEntregasV150()}catch(e){appMessage(e.message)}}
...
async function syncOrderStatusV150(pedidoId){if(!pedidoId)return;let rows=[];try{const q=await db.collection('ocorrencias_entrega').where('pedidoId','==',pedidoId).get();rows=q.docs.map(d=>d.data())}catch(e){const q=await db.collection('ocorrencias_entrega').get();rows=q.docs.map(d=>d.data()).filter(x=>x.pedidoId===pedidoId)}if(rows.length&&rows.every(x=>V150_FINAL_STATUSES.includes(x.status)))await db.collection('pedidos').doc(pedidoId).set({statusPedido:'concluido',concluidoEm:nowIso(),atualizadoEm:nowIso()},{merge:true})}
...
/* atualização de versão herdada removida pela V1.6.0-rc2.3.2-lanches-multiplos-produtos */
})();
