...
async function familyMovesV181(students){
  ...
  if(typeof window.loadFamilyHistoryWindowV221==='function'){
    let h=await window.loadFamilyHistoryWindowV221('report_family_open_values','movimentos_conta',students,{orderField:'criadoEm',pageSize:100,reset:true});
    while(!h.done)h=await window.loadFamilyHistoryWindowV221('report_family_open_values','movimentos_conta',students,{orderField:'criadoEm',pageSize:100});
    raw=h.merged;
  }else{
    const seen=new Map();
    for(const a of students||[]){
      for(const [field,array] of [['alunoId',false],['alunosIds',true]]){
        let cursor=null;
        while(true){let q=array?db.collection('movimentos_conta').where(field,'array-contains',a.id):db.collection('movimentos_conta').where(field,'==',a.id);q=q.limit(200);if(cursor)q=q.startAfter(cursor);const snap=await q.get();for(const d of snap.docs)seen.set(d.id,{id:d.id,...d.data()});if(snap.docs.length<200)break;cursor=snap.docs[snap.docs.length-1]}
      }
    }
    raw=[...seen.values()];
  }
