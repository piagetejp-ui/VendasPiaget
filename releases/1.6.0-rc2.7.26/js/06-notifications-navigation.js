...
async function markChangedEntitiesV221(action,data,responsavelId,auditId,entities,now){
  ...
  const base={ultimaAcao:action,ultimaAuditoriaId:auditId||null,ultimaAlteracaoEm:now,processada:false,versao:'1.6.0-rc2.7.26'};
  ...
  try{
    ...
    if(state.parentStudent&&!state.user){
      ...
      const packs=await Promise.all(ids.flatMap(id=>[
        db.collection(NOTIFICATIONS_V134).where('destinatariosAlunos','array-contains',id).limit(perQuery).get(),
        db.collection(NOTIFICATIONS_V134).where('alunoId','==',id).limit(perQuery).get()
      ]));
