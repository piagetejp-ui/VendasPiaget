...
window.financialAccountsSnapshotV167=async function(){
 if(!familyModelEnabledV167())return db.collection('contas_alunos').limit(400).get();
 const snap=await db.collection('contas_responsaveis').limit(400).get();
...
async function familyAuthProbeV179(payload){
 const a=authObjV130(),u=a?.currentUser;if(!u)throw new Error('A autenticação Firebase da família não foi concluída.');
 const tokenResult=await u.getIdTokenResult(true),claims=tokenResult?.claims||{},expectedSession=String(payload?.session?.sessionId||''),expectedResponsible=String(payload?.responsavelId||'');
 if(String(claims.role||'')!=='responsavel')throw new Error('A sessão segura foi criada sem o perfil de responsável.');
 if(String(claims.responsavelId||'')!==expectedResponsible)throw new Error('A sessão segura foi criada para uma família diferente da esperada.');
 if(String(claims.sessionId||'')!==expectedSession)throw new Error('A sessão segura do Firebase não corresponde à sessão ativa do Meu Piaget.');
 if(!Array.isArray(claims.alunosIds))throw new Error('A sessão segura não recebeu os vínculos de alunos.');
 const idToken=await u.getIdToken(true),r=await fetch('/api/familias?modulo=security',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Authorization':`Bearer ${idToken}`},body:JSON.stringify({acao:'family_auth_probe'})}),j=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(j.error||'O servidor não conseguiu confirmar a sessão segura da família.');
 return j;
}
async function activateFamilyFirebaseV176(payload){
 ...
 try{
   ...
   await familyAuthProbeV179(payload);
 }catch(e){
   console.error('family-auth-handshake',e);
...
async function loadFamilyCoreV167(){
 let configRead;
 try{configRead=await db.collection('configuracoes').doc('sistema').get()}catch(e){
   console.error('family-firestore-config',e);
   if(String(e?.code||'').includes('permission-denied')||String(e?.message||'').includes('Missing or insufficient permissions'))throw new Error('A sessão foi validada, mas o Firestore recusou a leitura inicial do Meu Piaget.');
   throw e;
 }
 const ds=await db.collection('disponibilidade_salgados').doc(dateKey()).get().catch(()=>({exists:false,data:()=>({})}));
 if(!configRead.exists)throw new Error('O Meu Piaget ainda não está disponível.');
 state.config=configRead.data()||{};
 /* RC2.7.24: o login da família carrega apenas o núcleo. Catálogos/estoques entram sob demanda. */
 ...
 state.v220FamilyCommerceLoaded=false;
...
window.openParentMovementsV151=openParentMovementsV151=async function(){
 ...
 try{
   const packs=await Promise.all(all.map(async a=>{const [m,c]=await Promise.all([db.collection('movimentos_conta').where('alunoId','==',a.id).limit(200).get().catch(()=>({docs:[]})),db.collection('pagamentos_checkout').where('alunoId','==',a.id).limit(80).get().catch(()=>({docs:[]}))]);return{a,m,c}}));
...
function operationalCountRowsV167(collections={}){const labels={vendas:'Vendas',pedidos:'Pedidos de cantina',pedidos_farda:'Pedidos de farda',pedidos_operacionais:'Pedidos operacionais',ocorrencias_entrega:'Entregas programadas',sessoes_caixa:'Sessões de caixa',periodos_responsabilidade_caixa:'Períodos de responsabilidade',movimentos_caixa:'Movimentos de caixa',movimentos_conta:'Movimentos financeiros',pagamentos:'Pagamentos',pagamentos_checkout:'Checkouts',vendas_online_links:'Links de venda online',lancamentos_manuais:'Lançamentos retroativos',disponibilidade_salgados:'Capacidade diária',fechamentos_semanais_familias:'Fechamentos familiares',historico_auditoria:'Auditoria',responsaveis_acesso:'Acessos Meu Piaget',sessoes_meu_piaget:'Sessões Meu Piaget',seguranca_rate_limit:'Proteção de acesso'};return Object.entries(collections).filter(([,v])=>Number(v?.total||0)>0).sort((a,b)=>Number(b[1]?.arquivar||0)-Number(a[1]?.arquivar||0)||Number(b[1]?.total||0)-Number(a[1]?.total||0)).slice(0,20).map(([k,v])=>`<div class="v167-reset-row"><span>${esc(labels[k]||String(k).replaceAll('_',' '))}<small style="display:block;color:#667085;margin-top:2px">${Number(v.preservar||0)} preservar${Number(v.retroativos||0)?` · ${Number(v.retroativos||0)} retroativo(s) real(is)`:''}${Number(v.semData||0)?` · ${Number(v.semData||0)} sem origem`:''}</small></span><strong>${Number(v.arquivar||0)} arquivar</strong></div>`).join('')}
function marcoSampleLabelV180(r={}){const labels={vendas:'Venda',pedidos:'Pedido de cantina',pedidos_farda:'Pedido de farda',pedidos_operacionais:'Pedido operacional',lancamentos_manuais:'Lançamento retroativo',ocorrencias_entrega:'Entrega programada',pagamentos:'Pagamento',pagamentos_checkout:'Checkout',pagamentos_venda:'Pagamento da venda',movimentos_conta:'Movimento financeiro',vendas_online_links:'Link de venda',transacoes_infinitepay:'Transação InfinitePay',sessoes_caixa:'Sessão de caixa'};return labels[r.colecao]||String(r.colecao||'Registro').replaceAll('_',' ')}
function marcoSamplesHtmlV180(rows=[],kind='keep'){if(!rows.length)return`<div class="empty-soft">${kind==='archive'?'Nenhum teste antigo encontrado nesta amostra.':kind==='retro'?'Nenhum lançamento retroativo real encontrado nesta amostra.':'Nenhuma operação da implantação encontrada nesta amostra.'}</div>`;return rows.map(r=>{const op=r.dataOperacao?` · operação ${humanDate(r.dataOperacao)}`:'',root=r.raiz?` · origem ${esc(r.raiz)}`:'';return`<div class="v167-reset-row"><span><strong>${esc(marcoSampleLabelV180(r))}</strong><small style="display:block;color:#667085;margin-top:2px">${esc(r.alunoNome||r.descricao||String(r.id).slice(-12))} · registro ${r.data?humanDate(r.data):'sem data'}${op}${root}</small></span><strong>${Number(r.valorCentavos||0)?fmt(Number(r.valorCentavos||0)):(kind==='archive'?'arquivar':'preservar')}</strong></div>`}).join('')}
window.renderImplantationPanelV167=async function(){const host=$('#v167ImplantationHost');if(!host)return;host.innerHTML='<div class="v151-inline-loader">Verificando implantação...</div>';try{const s=await implantationStatusV167();state.implantationV167=s;const cut=s.corte?.rotulo||'10/08/2026';host.innerHTML=`<div class="v167-implant-grid"><div class="v167-implant-step ${s.preparada?'done':''}"><strong>1. Base oficial 2026</strong><span>${s.preparada?'Preparada':'Aguardando preparação'}</span><small>${s.base.alunos} alunos · ${s.base.responsaveis} responsáveis financeiros</small>${!s.preparada?`<button class="btn btn-primary" onclick="prepareOfficialBaseV167()">Preparar base oficial</button>`:''}</div><div class="v167-implant-step ${s.cpfPendentes.length?'warn':'done'}"><strong>2. Cadastros de acesso</strong><span>${s.cpfPendentes.length?`${s.cpfPendentes.length} CPF pendente`:'CPFs prontos para acesso'}</span>${s.cpfPendentes.map(r=>`<div class="v167-cpf-fix"><span>${esc(r.nome)}</span><button class="btn btn-light" onclick="openCpfCorrectionV167('${esc(r.id)}','${esc(r.nome)}')">Corrigir CPF</button></div>`).join('')}</div><div class="v167-implant-step ${s.operacaoOficial?'done':''}"><strong>3. Marco inicial da operação</strong><span>${s.operacaoOficial?`Corte registrado em ${cut}`:`Origem real a partir de ${cut}`}</span><small>${s.operacaoOficial?'Dados reais preservados pelo critério de origem.':`Testes cuja origem é até 09/08 serão arquivados. Retroativos reais registrados em ${cut}+ serão preservados.`}</small>${!s.operacaoOficial?`<button class="btn btn-orange" ${!s.preparada||s.cpfPendentes.length?'disabled':''} onclick="previewOfficialResetV167()">Revisar corte do Marco Zero</button>`:''}</div></div>`}catch(e){host.innerHTML=`<div class="alert danger">${esc(e.message)}</div>`}};
...
window.previewOfficialResetV167=async function(){try{const d=await staffApiV167('/api/implantacao',{acao:'preview_reset'}),cut=d.dataCorte?.rotulo||'10/08/2026';openModal('Marco Zero — revisão por origem',`<div class="alert warn"><strong>O Marco Zero agora decide pela origem da operação.</strong><br>Uma programação criada antes de ${cut} será arquivada mesmo que a entrega esteja marcada para depois. Já uma pendência real de data antiga registrada pela Secretaria em ${cut} ou depois será preservada como retroativo real.</div><div class="v167-reset-summary"><div><small>Testes antigos — arquivar</small><strong>${Number(d.resumo?.arquivar||0)}</strong></div><div><small>Dados reais — preservar</small><strong>${Number(d.resumo?.preservar||0)}</strong></div><div><small>Retroativos reais preservados</small><strong>${Number(d.resumo?.retroativos||0)}</strong></div><div><small>Saldo reconstruído</small><strong>${fmt(Number(d.contas?.saldoAposCorteCentavos||0))}</strong></div><div><small>Movimentos reais considerados</small><strong>${Number(d.contas?.movimentosConsiderados||0)}</strong></div><div><small>Movimentos retroativos considerados</small><strong>${Number(d.contas?.movimentosRetroativos||0)}</strong></div></div>${Number(d.resumo?.semData||0)?`<div class="alert"><strong>${Number(d.resumo.semData)} registro(s) sem origem confiável serão preservados por segurança.</strong><br>Esses registros não entram automaticamente na reconstrução financeira.</div>`:''}<div class="v167-info-box" style="margin-bottom:12px"><strong>Base e reconciliação</strong><span>${Number(d.baseOficial?.alunos||0)} alunos · ${Number(d.baseOficial?.responsaveis||0)} responsáveis · ${Number(d.baseOficial?.vinculos||0)} vínculos<br>Saldo atual: ${fmt(Number(d.contas?.saldoAtualCentavos||0))} → saldo reconstruído: ${fmt(Number(d.contas?.saldoAposCorteCentavos||0))}<br>Salgados: ${Number(d.derivados?.ocorrenciasSalgadosConsideradas||0)} ocorrência(s) real(is) considerada(s) · Fardamento: ${Number(d.derivados?.unidadesFardaComprometidas||0)} unidade(s) comprometida(s). O estoque físico não será recalculado.</span></div><div class="card-title" style="margin:12px 0 6px">Implantação — preservar</div><div class="v167-reset-list">${marcoSamplesHtmlV180(d.amostrasImplantacao||[],'keep')}</div><div class="card-title" style="margin:14px 0 6px">Retroativos reais — preservar</div><div class="v167-reset-list">${marcoSamplesHtmlV180(d.amostrasRetroativas||[],'retro')}</div><div class="card-title" style="margin:14px 0 6px">Testes anteriores — arquivar</div><div class="v167-reset-list">${marcoSamplesHtmlV180(d.amostrasArquivar||[],'archive')}</div><div class="card-title" style="margin:14px 0 6px">Resumo por coleção</div><div class="v167-reset-list">${operationalCountRowsV167(d.colecoes)||'<div class="empty-soft">Não há registros operacionais nas coleções monitoradas.</div>'}</div><div class="alert" style="margin-top:12px"><strong>Execute com a operação momentaneamente parada.</strong><br>O corte também reconciliará a capacidade futura de salgados e as reservas/produção de farda usando somente operações preservadas. O estoque físico não será alterado.</div><div class="fg" style="margin-top:14px"><label>Para confirmar, digite exatamente:</label><div class="reset-link-box-v130">INICIAR OPERAÇÃO REAL</div><input id="v167ResetConfirmation" class="fi" autocomplete="off" placeholder="INICIAR OPERAÇÃO REAL"></div><button class="btn btn-red" style="width:100%" onclick="executeOfficialResetV167()">Criar backup e aplicar Marco Zero por origem</button>`)}catch(e){appMessage(e.message)}};
window.executeOfficialResetV167=async function(){const c=$('#v167ResetConfirmation')?.value||'';if(c!=='INICIAR OPERAÇÃO REAL')return appMessage('Digite exatamente INICIAR OPERAÇÃO REAL.');if(!await appConfirm('O Marco Zero arquivará testes conforme a ORIGEM da operação. Programações antigas com entregas futuras serão removidas; lançamentos retroativos reais registrados desde 10/08 serão preservados. As contas, capacidade de salgados e reservas de farda serão reconciliadas.',{title:'Aplicar o Marco Zero por origem?',confirmLabel:'Aplicar corte e iniciar piloto',cancelLabel:'Cancelar',danger:true}))return;try{const d=await staffApiV167('/api/implantacao',{acao:'iniciar_operacao',confirmacao:c});closeModal();toast(`Marco operacional registrado. ${Number(d.backup?.totalArquivados||0)} registro(s) de teste arquivado(s).`);await loadCore();navigate('config')}catch(e){appMessage(e.message)}};
...
window.PiagetFamilyV167={activeFamilyModeV167,familyModelEnabledV167,staffApiV167,familyApiV167,securityApiV176:familySecurityApiV176,authProbeV179:familyAuthProbeV179,renderFamilyResetFromQueryV167};
