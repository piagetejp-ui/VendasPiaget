...
const FAMILY_SESSION_KEY='vp_family_session';
...
function familySessionTokenV167(){return localStorage.getItem(FAMILY_SESSION_KEY)||''}

/* Toda chamada operacional feita dentro do Meu Piaget carrega a sessão familiar.
   O backend ainda mantém os links públicos da Secretaria independentes deste login. */
const BASE_FETCH_V167=window.fetch.bind(window);
window.fetch=function(input,init={}){
 try{
   const raw=typeof input==='string'?input:(input?.url||''),url=new URL(raw,location.href),token=familySessionTokenV167();
   if(activeFamilyModeV167()&&token&&url.origin===location.origin&&url.pathname.startsWith('/api/')){
     const headers=new Headers(init.headers||(typeof input!=='string'?input?.headers:undefined)||{});
     headers.set('X-Piaget-Family-Session',token);
     init={...init,headers};
   }
 }catch(_){}
...
async function familyApiV167(acao,data={}){
 const r=await fetch('/api/acesso-meu-piaget',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({acao,...data})}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'Não foi possível concluir agora.');return j;
}
...
function applyFamilyPayloadV167(payload,token){
 if(token)localStorage.setItem(FAMILY_SESSION_KEY,token);
...
window.familyPasswordLoginV167=async function(){const host=$('#familyLoginMsgV167');try{if(host)host.innerHTML='<div class="v151-inline-loader">Entrando...</div>';const d=await familyApiV167('login',{cpf:$('#familyCpfV167')?.value||'',senha:$('#familyPassV167')?.value||''});applyFamilyPayloadV167(d,d.session?.token);await renderParentPortal()}catch(e){if(host)host.innerHTML=`<div class="alert danger">${esc(e.message)}</div>`}};
...
window.bootFamilyV167=async function(){
 ...
 try{state.students=[];await loadFamilyCoreV167();if($('#bootProgress'))$('#bootProgress').style.width='75%';if(await renderFamilyResetFromQueryV167())return;const token=familySessionTokenV167();if(token){try{const d=await familyApiV167('sessao',{sessionToken:token});applyFamilyPayloadV167(d,token);if($('#bootProgress'))$('#bootProgress').style.width='100%';return renderParentPortal()}catch(_){localStorage.removeItem(FAMILY_SESSION_KEY)}}if($('#bootProgress'))$('#bootProgress').style.width='100%';renderMeuPiagetLoginV167()}catch(e){console.error('bootFamily',e);if($('#bootText'))$('#bootText').textContent='Não foi possível abrir o Meu Piaget.';const h=$('#bootError');h?.classList.remove('hidden');if(h)h.innerHTML=esc(e.message||'Tente novamente em instantes.');const b=$('#retryBtn');b?.classList.remove('hidden');if(b)b.onclick=()=>bootFamilyV167()}
};
...
window.saveParentAuthorizationV151=saveParentAuthorizationV151=async function(){const a=state.parentStudent,max=familyMaxLimitV167(),on=$('#v151AuthSwitch').classList.contains('on'),limit=Math.max(0,Math.min(max,Math.round(Number($('#v151AuthLimit').value||0)*100))),acc=await getAccount(a.id),net=netV151(acc);await financialAccountRefV167(a.id).set({autorizadoSemSaldo:on,limiteFiadoCentavos:limit,bloqueadoPorLimite:limit>0&&net<0&&Math.abs(net)>=limit,atualizadoEm:nowIso()},{merge:true});state.parentFamily.conta={...state.parentFamily.conta,autorizadoSemSaldo:on,limiteFiadoCentavos:limit};await audit('autorizacao_responsavel',{alunoId:a.id,responsavelId:familyResponsibleIdV167(a),autorizado:on,limiteCentavos:limit,versao:VERSION}).catch(()=>{});closeModal();toast('Autorização salva.');renderParentPortal()};

window.parentLogoutClean=parentLogoutClean=async function(){const token=familySessionTokenV167();if(token)familyApiV167('sair',{sessionToken:token}).catch(()=>{});localStorage.removeItem(FAMILY_SESSION_KEY);localStorage.removeItem(FAMILY_STUDENT_KEY);state.parentFamily=null;state.parentStudent=null;state.parentChallenge=null;$('#notifBellV134')?.remove();renderMeuPiagetLoginV167()};
...
window.parentUrl=parentUrl=function(a){const configured=String(state.config?.urlMeuPiaget||'/meu-piaget.html');const url=new URL(configured,location.origin);if(a?.matricula)url.searchParams.set('matricula',a.matricula);return url.toString()};
...
setFamilyHeaderV167();
window.PiagetFamilyV167={activeFamilyModeV167,familyModelEnabledV167,staffApiV167,familyApiV167,renderFamilyResetFromQueryV167};
})();
