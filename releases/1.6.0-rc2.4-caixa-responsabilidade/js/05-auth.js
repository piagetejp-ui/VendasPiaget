...
async function boot(){
  ...
  try{
    ...
    if(!cfg.exists)throw new Error('A base do sistema não foi encontrada. Configure o Firestore antes de acessar.')
  ...
  }catch(e){console.error(e);setConnection('Sem acesso ao Firestore',false);$('#bootText').textContent='Não foi possível acessar o Firestore.';$('#bootError').classList.remove('hidden');$('#bootError').innerHTML=`${esc(e.message)}<br><br>Verifique a conexão, as regras do Firestore e o cadastro da equipe em Usuários e acessos.`;$('#retryBtn').classList.remove('hidden')}
}
...
async function enterAuthenticatedInternalV130(firebaseUser,{silent=false}={}){
  ...
  if(!profile||profile.ativo===false){if(!silent)appMessage('Este e-mail autenticou no Firebase, mas não há perfil ativo no sistema.');await authObjV130().signOut().catch(()=>{});return false}
...
/* V1.6.0-rc2.4-caixa-responsabilidade — logout canônico do botão superior. */
