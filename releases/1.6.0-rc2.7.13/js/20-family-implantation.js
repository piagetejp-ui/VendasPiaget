...
async function loadFamilyCoreV167(){
 ...
 try{configRead=await db.collection('configuracoes').doc('sistema').get()}catch(e){
   ...
   if(String(e?.code||'').includes('permission-denied')||String(e?.message||'').includes('Missing or insufficient permissions'))throw new Error('A sessão foi validada, mas o Firestore recusou a leitura inicial do Meu Piaget. Publique também as Firestore Rules da RC2.7.13 e tente novamente.');
   throw e;
 }
