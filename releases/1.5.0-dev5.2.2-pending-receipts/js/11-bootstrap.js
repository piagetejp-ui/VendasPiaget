...
/* Escola Piaget — V1.5.0-dev5.2.2-pending-receipts
   Inicialização única. Nenhuma função histórica é redirecionada após o login. */
const APP_VERSION_CLEAN='1.5.0-dev5.2.2-pending-receipts';
...
let financialMaintenanceStartedClean=false;

async function runSilentFinancialMaintenanceClean(){
  if(financialMaintenanceStartedClean)return;
  financialMaintenanceStartedClean=true;
  try{
    const controller=new AbortController();
    const timer=setTimeout(()=>controller.abort(),20000);
    const response=await fetch('/api/sincronizar-financeiro',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}',cache:'no-store',signal:controller.signal});
    clearTimeout(timer);
    const data=await response.json().catch(()=>({}));
    if(!response.ok||data.ok===false)console.warn('financial-maintenance',data.error||'Sincronização pendente.');
  }catch(error){
    console.warn('financial-maintenance',error?.message||error);
  }
}
...
async function startApplicationClean(){
  ...
  runSilentFinancialMaintenanceClean();
...
window.runSilentFinancialMaintenanceClean=runSilentFinancialMaintenanceClean;
