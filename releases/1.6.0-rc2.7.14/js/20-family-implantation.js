...
async function loadFamilyCoreV167(){
 ...
 try{configRead=await db.collection('configuracoes').doc('sistema').get()}catch(e){
   ...
   if(String(e?.code||'').includes('permission-denied')||String(e?.message||'').includes('Missing or insufficient permissions'))throw new Error('A sessão foi validada, mas o Firestore recusou a leitura inicial do Meu Piaget. Publique também as Firestore Rules da RC2.7.14 e tente novamente.');
   throw e;
 }
...
window.previewOfficialResetV167=async function(){try{const d=await staffApiV167('/api/implantacao',{acao:'preview_reset'}),cut=d.dataCorte?.rotulo||'10/08/2026',before='09/08/2026';openModal('Marco Zero — revisão do corte',`<div class="alert warn"><strong>O Marco Zero será aplicado por data, não como reset global.</strong><br>Registros até ${before} serão arquivados em backup. Tudo que estiver em ${cut} ou depois será preservado como dado da implantação piloto. As contas familiares não serão simplesmente zeradas: o saldo será recalculado pelas movimentações a partir do corte.</div><div class="v167-reset-summary"><div><small>Arquivar como teste antigo</small><strong>${Number(d.resumo?.arquivar||0)}</strong></div><div><small>Preservar na implantação</small><strong>${Number(d.resumo?.preservar||0)}</strong></div><div><small>Saldo consolidado após o corte</small><strong>${fmt(Number(d.contas?.saldoAposCorteCentavos||0))}</strong></div><div><small>Movimentos pós-corte considerados</small><strong>${Number(d.contas?.movimentosConsiderados||0)}</strong></div></div>${Number(d.resumo?.semData||0)?`<div class="alert"><strong>${Number(d.resumo.semData)} registro(s) sem data serão preservados por segurança.</strong><br>Nenhum documento sem data confiável será apagado automaticamente.</div>`:''}<div class="v167-info-box" style="margin-bottom:12px"><strong>Base preservada</strong><span>${Number(d.baseOficial?.alunos||0)} alunos · ${Number(d.baseOficial?.responsaveis||0)} responsáveis · ${Number(d.baseOficial?.vinculos||0)} vínculos<br>Saldo atual antes do corte: ${fmt(Number(d.contas?.saldoAtualCentavos||0))} → saldo reconstruído: ${fmt(Number(d.contas?.saldoAposCorteCentavos||0))}</span></div><div class="card-title" style="margin:12px 0 6px">Operações pós-corte que serão preservadas</div><div class="v167-reset-list">${preservedSamplesHtmlV167(d.amostrasPreservadas||[])}</div><div class="card-title" style="margin:14px 0 6px">Resumo por coleção</div><div class="v167-reset-list">${operationalCountRowsV167(d.colecoes)||'<div class="empty-soft">Não há registros operacionais nas coleções monitoradas.</div>'}</div><div class="alert" style="margin-top:12px"><strong>Execute o corte com a operação momentaneamente parada.</strong><br>Não conclua uma venda ou pagamento enquanto o Marco Zero estiver sendo aplicado.</div><div class="fg" style="margin-top:14px"><label>Para confirmar, digite exatamente:</label><div class="reset-link-box-v130">INICIAR OPERAÇÃO REAL</div><input id="v167ResetConfirmation" class="fi" autocomplete="off" placeholder="INICIAR OPERAÇÃO REAL"></div><button class="btn btn-red" style="width:100%" onclick="executeOfficialResetV167()">Criar backup e aplicar corte de ${cut}</button>`)}catch(e){appMessage(e.message)}};
window.executeOfficialResetV167=async function(){const c=$('#v167ResetConfirmation')?.value||'';if(c!=='INICIAR OPERAÇÃO REAL')return appMessage('Digite exatamente INICIAR OPERAÇÃO REAL.');if(!await appConfirm('Somente os registros anteriores a 10/08/2026 serão arquivados. As operações de 10/08/2026 em diante serão preservadas e as contas serão recalculadas com base nas movimentações pós-corte.',{title:'Aplicar o Marco Zero de 10/08/2026?',confirmLabel:'Aplicar corte e iniciar piloto',cancelLabel:'Cancelar',danger:true}))return;try{const d=await staffApiV167('/api/implantacao',{acao:'iniciar_operacao',confirmacao:c});closeModal();toast(`Marco operacional registrado. ${Number(d.backup?.totalArquivados||0)} registro(s) antigo(s) arquivado(s).`);await loadCore();navigate('config')}catch(e){appMessage(e.message)}};
