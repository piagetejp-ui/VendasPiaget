...
function notificationVisibleV134(n){
  if(state.parentStudent&&!state.user){const id=state.parentStudent.id;return (n.destinatariosAlunos||[]).includes(id)||((n.destinatariosPerfis||[]).includes('responsavel')&&String(n.alunoId||'')===String(id))}
...
function buildSidebar(){
  ...
  $('#sidebar').innerHTML=`<div class="nav-label">Operação</div>${items.map(i=>`<button class="nav-btn" data-page="${i[0]}" onclick="navigate('${i[0]}')">${iconV120(i[1])}<span>${i[2]}</span></button>`).join('')}<div class="nav-foot"><strong>${esc(state.user?.nome||'-')}</strong><br>${roleLabelV130(state.user?.perfil)}<br><br>${state.user?.authMode==='firebase'?'Login autenticado pelo Firebase.':'Acesso interno do sistema.'}<br><span class="system-version-clean">Versão ${esc(document.querySelector('meta[name=application-version]')?.content||'')}</span></div>`;
