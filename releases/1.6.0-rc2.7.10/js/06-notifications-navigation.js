...
async function loadNotificationsForUserV134(limit=120){
  try{
    if(state.parentStudent&&!state.user){
      const ids=(state.parentFamily?.alunos||[state.parentStudent]).map(a=>String(a.id)),packs=await Promise.all(ids.flatMap(id=>[db.collection(NOTIFICATIONS_V134).where('destinatariosAlunos','array-contains',id).get().catch(()=>({docs:[]})),db.collection(NOTIFICATIONS_V134).where('alunoId','==',id).get().catch(()=>({docs:[]}))]));
      const map=new Map();for(const snap of packs)for(const d of snap.docs)map.set(d.id,{id:d.id,...d.data()});return [...map.values()].filter(notificationVisibleV134).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||''))).slice(0,limit);
    }
    const snap=await db.collection(NOTIFICATIONS_V134).orderBy('criadoEm','desc').limit(limit).get();return snap.docs.map(d=>({id:d.id,...d.data()})).filter(notificationVisibleV134)
...
async function refreshNotificationBadgeV134(){
  ...
  const rows=await loadNotificationsForUserV134(100),unread=rows.filter(n=>!(n.lidoPor||[]).includes(uid)).length,pending=rows.filter(n=>notificationEffectiveStatusV165(n)==='pendente').length;
  countEl.textContent=unread>99?'99+':String(unread);countEl.classList.toggle('on',unread>0);bell.title=unread?`${unread} notificação(ões) não lida(s)${pending?` · ${pending} pendência(s)`:''}`:'Sem notificações novas';
}
