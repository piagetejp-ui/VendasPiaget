...
async function verifyCheckoutPaymentV141(orderNsu,transactionNsu='',invoiceSlug=''){try{const resp=await fetch('/api/verificar-pagamento',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_nsu:orderNsu,transaction_nsu:transactionNsu,slug:invoiceSlug})});const data=await resp.json().catch(()=>({}));if(!resp.ok)throw new Error(data.error||'Erro ao atualizar pagamento.');toast(data.paid?(data.applicationPending?'Pagamento localizado; processamento pendente.':'Pagamento confirmado e aplicado.'):'Pagamento ainda não confirmado.');renderCobrancas();}catch(e){alert(e.message)}}
...
/* V1.5.0-dev5.2.1-financial-hotfix — tentativa idempotente e feedback de checkout */
