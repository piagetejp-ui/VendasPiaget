...
async function pendingCheckoutsV141(alunoId){const active=new Set(['preparando_link','aguardando_pagamento','erro_timeout','erro_gerar_link','erro_sem_url','pagamento_localizado_processando','pagamento_localizado_aguardando_processamento','sem_retorno_infinitepay']);try{if(!state.user&&state.parentStudent&&typeof window.familyDataApiV228==='function'){const out=await window.familyDataApiV228('query_records',{collection:CHECKOUT_COLLECTION_V141,arrayField:'alunosIds',mode:'full',orderField:'criadoEm'}),rows=Array.isArray(out.rows)?out.rows:[];return rows.filter(x=>String(x.alunoId||'')===String(alunoId)&&active.has(x.status||'aguardando_pagamento')).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||'')))}const rows=[];let cursor=null;while(true){let q=db.collection(CHECKOUT_COLLECTION_V141).where('alunoId','==',alunoId).limit(100);if(cursor)q=q.startAfter(cursor);const snap=await q.get();rows.push(...snap.docs.map(d=>({id:d.id,...d.data()})));if(snap.docs.length<100)break;cursor=snap.docs[snap.docs.length-1]}return rows.filter(x=>active.has(x.status||'aguardando_pagamento')).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||'')))}catch(e){console.warn('pendingCheckoutsV141',e);try{appMessage('Não foi possível conferir os pagamentos pendentes. A operação foi interrompida para evitar uma cobrança duplicada.',{title:'Falha ao conferir cobranças',tone:'danger'})}catch(_){}throw e}}
...
async function pendingCheckoutsV143(alunoId){const active=new Set(['preparando_link','aguardando_pagamento','erro_timeout','erro_gerar_link','erro_sem_url','pagamento_localizado_processando','pagamento_localizado_aguardando_processamento','sem_retorno_infinitepay']);try{if(!state.user&&state.parentStudent&&typeof window.familyDataApiV228==='function'){const out=await window.familyDataApiV228('query_records',{collection:CHECKOUT_COLLECTION_V143,arrayField:'alunosIds',mode:'full',orderField:'criadoEm'}),rows=Array.isArray(out.rows)?out.rows:[];return rows.filter(x=>String(x.alunoId||'')===String(alunoId)&&active.has(x.status||'aguardando_pagamento')).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||'')))}const rows=[];let cursor=null;while(true){let q=db.collection(CHECKOUT_COLLECTION_V143).where('alunoId','==',alunoId).limit(100);if(cursor)q=q.startAfter(cursor);const snap=await q.get();rows.push(...snap.docs.map(d=>({id:d.id,...d.data()})));if(snap.docs.length<100)break;cursor=snap.docs[snap.docs.length-1]}return rows.filter(x=>active.has(x.status||'aguardando_pagamento')).sort((a,b)=>String(b.criadoEm||'').localeCompare(String(a.criadoEm||'')));}catch(e){console.warn('pendingCheckoutsV143',e);try{appMessage('Não foi possível conferir os pagamentos pendentes. A operação foi interrompida para evitar uma cobrança duplicada.',{title:'Falha ao conferir cobranças',tone:'danger'})}catch(_){}throw e}}
...
/* V1.6.0-rc2.7.41 — tentativa idempotente e feedback de checkout */
