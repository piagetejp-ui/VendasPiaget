...
/* Escola Piaget — V1.6.0-rc2.7.2-familia-compartilhada
   Inicialização única. Nenhuma função histórica é redirecionada após o login. */
const APP_VERSION_CLEAN='1.6.0-rc2.7.2-familia-compartilhada';
...
async function startApplicationClean(){
  ...
  const familyMode=(typeof window.PiagetFamilyV167?.activeFamilyModeV167==='function'&&window.PiagetFamilyV167.activeFamilyModeV167());
  if(familyMode){await bootFamilyV167();return}
  await boot();
}
