...
async function boot(){
  show('bootScreen');$('#bootError').classList.add('hidden');$('#retryBtn').classList.add('hidden');$('#bootText').textContent='Preparando dados e acessos...';$('#bootProgress').style.width='25%';
  try{
    const cfg=await db.collection('configuracoes').doc('sistema').get();setConnection('Sistema online',true);$('#bootProgress').style.width='55%';
    if(!cfg.exists)throw new Error('A base do sistema não foi encontrada. Entre em contato com a gestão responsável pelo sistema.')
...
async function enterAuthenticatedInternalV130(firebaseUser,{silent=false}={}){
  ...
  if(!profile||profile.ativo===false){if(!silent)appMessage('Este acesso não possui um perfil ativo no sistema.');await authObjV130().signOut().catch(()=>{});return false}
  await db.collection(ACCESS_AUTH_V130).doc(firebaseUser.uid).set({uid:firebaseUser.uid,usuarioAcessoId:profile.id,nome:profile.nome,email,perfil:profile.perfil,turnos:profile.turnos||[],permissoes:profile.permissoes||basePermissionsV130(profile.perfil),caixaPermissao:accessCashPermissionV164(profile),ativo:profile.ativo!==false,atualizadoEm:nowIso()},{merge:true}).catch(()=>{});
...
/* V1.6.0-rc2.7.2-familia-compartilhada — logout canônico do botão superior. */
