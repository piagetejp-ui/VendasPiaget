...
async function renderDashboardV141(){
  ...
  const [vs,os,acs,fs,pcs]=await Promise.all([db.collection('vendas').where('dataChave','==',today).get().catch(()=>({docs:[]})),db.collection('ocorrencias_entrega').where('dataChave','==',today).get().catch(()=>({docs:[]})),financialAccountsSnapshotV167(),db.collection('pedidos_farda').get().catch(()=>({docs:[]})),recentCheckoutsV141(40)]);
...
/* V1.6.0-rc2.7.2-familia-compartilhada — tentativa idempotente e feedback de checkout */
