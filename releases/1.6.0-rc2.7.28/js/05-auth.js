...
function permissionLabelV166(key){return ({dashboard:'Resumo',atendimento:'Atendimento da Cantina',entregas:'Pedidos do dia',caixa:'Caixa',consulta:'Consulta de alunos',alunos:'Alunos e contas',vendas:'Vendas',cobrancas:'Cobranças',pedidos:'Pedidos',fardas:'Fardamento',produtos:'Catálogo de vendas',config:'Configurações',auditoria:'Auditoria',usuarios:'Usuários e acessos',notificacoes:'Notificações'}[key]||String(key||'').replaceAll('_',' '))}
function permissionListV166(perfil){return basePermissionsV130(perfil).filter(x=>x!=='fardas').map(permissionLabelV166)}

function defaultCashPermissionV164(perfil){return ['admin','gestao'].includes(perfil)?'gestor':['secretaria','cantina'].includes(perfil)?'operador':'nenhum'}
function accessCashPermissionV164(user={}){return user.caixaPermissao||defaultCashPermissionV164(user.perfil)}
function initialAccessUsersV130(){return [
  {id:'lucas',nome:'Lucas Viana',email:'',perfil:'admin',turnos:['manha','tarde'],caixaPermissao:'gestor',ativo:true,permissoes:basePermissionsV130('admin'),observacao:'Administrador e gestor de caixa'},
  {id:'daniele',nome:'Daniele',email:'',perfil:'secretaria',turnos:['manha'],caixaPermissao:'operador',ativo:true,permissoes:basePermissionsV130('secretaria'),observacao:'Operadora de Secretaria'},
  {id:'evanda',nome:'Evanda',email:'',perfil:'secretaria',turnos:['tarde'],caixaPermissao:'operador',ativo:true,permissoes:basePermissionsV130('secretaria'),observacao:'Operadora de Secretaria'},
  {id:'ruan',nome:'Ruan de Jesus',email:'',perfil:'cantina',turnos:['manha','tarde'],caixaPermissao:'operador',ativo:true,permissoes:basePermissionsV130('cantina'),observacao:'Operador da Cantina'}
]}
async function migrateV130Access(){
  const cfgRef=db.collection('configuracoes').doc('sistema'),cfg=await cfgRef.get().catch(()=>null),existing=await db.collection(ACCESS_USERS_V130).get().catch(()=>({docs:[]})),byId=new Map(existing.docs.map(d=>[d.id,{ref:d.ref,data:d.data()||{}}])),batch=db.batch(),now=nowIso();
  let changed=false;
  for(const u of initialAccessUsersV130()){
    const current=byId.get(u.id);
    if(!current){batch.set(db.collection(ACCESS_USERS_V130).doc(u.id),{...u,authUid:null,criadoEm:now,atualizadoEm:now});changed=true;continue}
    if(!current.data.caixaPermissao){batch.set(current.ref,{caixaPermissao:u.caixaPermissao,atualizadoEm:now},{merge:true});changed=true}
  }
  if(!cfg?.data()?.v130Migrado){batch.set(cfgRef,{versao:V130_VERSION,v130Migrado:true,acessoInterno:'firebase_email_senha',responsavelSenha:true,resetResponsavel:'link_temporario_secretaria',atualizadoEm:now},{merge:true});changed=true}
  if(changed)await batch.commit().catch(e=>console.warn('migrateV130Access',e));
}
...
async function staffBootstrapV176(firebaseUser){
  const token=await firebaseUser.getIdToken(true),r=await fetch('/api/familias?modulo=security',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({acao:'staff_bootstrap'})}),j=await r.json().catch(()=>({}));
  if(!r.ok)throw new Error(j.error||'Não foi possível validar o acesso da equipe.');return j.profile;
}
async function boot(){
  show('bootScreen');$('#bootError').classList.add('hidden');$('#retryBtn').classList.add('hidden');$('#bootText').textContent='Validando acesso da equipe...';$('#bootProgress').style.width='30%';
  try{
    const authUser=await waitAuthReadyV130();$('#bootProgress').style.width='55%';
    if(authUser){const ok=await enterAuthenticatedInternalV130(authUser,{silent:true});if(ok){$('#bootProgress').style.width='100%';return}}
    $('#bootProgress').style.width='100%';renderAccessLandingV130();show('roleScreen');setConnection('Aguardando login',true);
  }catch(e){console.error(e);setConnection('Sistema indisponível',false);$('#bootText').textContent='Não foi possível acessar o sistema.';$('#bootError').classList.remove('hidden');$('#bootError').innerHTML=`${esc(e.message)}<br><br>Verifique sua conexão e, se o problema continuar, revise os acessos da equipe.`;$('#retryBtn').classList.remove('hidden')}
}
function renderAccessLandingV130(msg=''){
  $('#roleScreen').innerHTML=`<div class="role-box"><div class="access-logo-v130"><img src="${BRAND_V122?.light||BRAND_V120.icon}" alt="Símbolo Escola Piaget"><div><strong>Escola Piaget</strong><span>Vendas, cantina e atendimento</span></div></div>${msg?`<div class="alert ${msg.includes('erro')||msg.includes('não')?'danger':''}" style="margin-bottom:12px">${esc(msg)}</div>`:''}<div class="access-v130"><div class="access-panel-v130"><h3>Acesso da equipe</h3><p>Entre com o e-mail e a senha da equipe.</p><div class="fg"><label>E-mail</label><input id="loginEmailV130" class="fi" type="email" autocomplete="username" placeholder="nome@escolapiaget.com.br"></div><div class="fg"><label>Senha</label><input id="loginPassV130" class="fi" type="password" autocomplete="current-password" placeholder="Senha de acesso"></div><button class="btn btn-primary" style="width:100%" onclick="loginInternalV130()">Entrar no sistema</button><div class="actions" style="margin-top:10px"><button class="btn btn-outline" onclick="sendInternalPasswordResetV130()">Enviar redefinição de senha</button></div></div><div class="access-panel-v130"><h3>Meu Piaget</h3><p>Acesse a conta do aluno com matrícula ou senha.</p><button class="btn btn-orange" style="width:100%" onclick="enterParent()">Acessar conta do aluno</button>${infoButton('Acesso do responsável','No primeiro acesso, o responsável valida a matrícula e pode criar uma senha. Redefinições são liberadas pela secretaria.')}</div></div></div>`;
  applyBrandingV122?.();
}
async function loginInternalV130(){
  const email=normalizeEmailV130($('#loginEmailV130')?.value),pass=$('#loginPassV130')?.value||'';if(!email||!pass)return appMessage('Informe e-mail e senha.');
  try{const cred=await authObjV130().signInWithEmailAndPassword(email,pass);await enterAuthenticatedInternalV130(cred.user)}catch(e){console.error(e);renderAccessLandingV130('Não foi possível entrar. Verifique o e-mail, a senha e se o seu acesso está ativo.')}
}
async function sendInternalPasswordResetV130(){const email=normalizeEmailV130($('#loginEmailV130')?.value);if(!email)return appMessage('Informe o e-mail para enviar a redefinição.');try{await authObjV130().sendPasswordResetEmail(email);toast('E-mail de redefinição enviado.')}catch(e){appMessage('Não foi possível enviar a redefinição: '+e.message)}}
async function enterAuthenticatedInternalV130(firebaseUser,{silent=false}={}){
  try{
    const profile=await staffBootstrapV176(firebaseUser),email=normalizeEmailV130(firebaseUser.email||profile.email||'');
    ...
    $('#bootText')&&($('#bootText').textContent='Carregando dados da operação...');$('#bootProgress')&&($('#bootProgress').style.width='72%');
    await loadCore();await migrateV130Access();
    $('#userPill').textContent=`${state.user.nome} · ${roleLabelV130(state.user.perfil)}`;$('#userPill').classList.remove('hidden');$('#switchUserBtn').textContent='Sair';$('#switchUserBtn').classList.remove('hidden');$('#connectionPill')?.classList.remove('hidden');setConnection('Sistema online',true);show('appShell');buildSidebar();navigate('dashboard');await audit('login_interno',{usuarioAcessoId:profile.id,authUid:firebaseUser.uid,email});return true;
  }catch(e){console.error('staff-bootstrap',e);if(!silent)appMessage(e.message||'Este acesso não possui um perfil ativo no sistema.');await authObjV130().signOut().catch(()=>{});return false}
}
...
async function renderUsuariosAcessosV130(){
  ...
  $('#mainContent').innerHTML=pageHeader('Usuários e acessos','Perfis internos, permissões e reset de acesso dos responsáveis.',`<button class="btn btn-primary" onclick="openUserProfileV130()">Novo perfil</button>`)+`<div class="grid g2"><div class="card"><div class="card-head"><div class="card-title">Equipe interna</div></div><div class="card-body"><div class="alert"><strong>Acessos da equipe:</strong> mantenha o e-mail de login igual ao cadastrado para o usuário. No primeiro acesso, o vínculo é feito automaticamente.</div><div style="margin-top:12px">${users.map(u=>userRowHtmlV130(u)).join('')||'<div class="empty">Nenhum usuário configurado.</div>'}</div></div></div><div class="card"><div class="card-head"><div class="card-title">Permissões por perfil</div></div><div class="card-body"><div class="perm-grid-v130">${['admin','gestao','secretaria','cantina'].map(p=>`<div class="perm-chip-v130"><strong>${roleLabelV130(p)}</strong><br>${permissionListV166(p).join(', ')}</div>`).join('')}</div><div class="alert"><strong>Hierarquia do caixa:</strong> operadores justificam divergências; perfis gestores dão a decisão final.</div></div></div></div><div class="card" style="margin-top:14px"><div class="card-head"><div class="card-title">Acesso dos responsáveis</div></div><div class="card-body"><div class="form-row"><div class="fg"><label>Buscar aluno</label><input id="parentAccessSearchV130" class="fi" placeholder="Nome, matrícula ou turma" oninput="renderResponsibleAccessSearchV130()"></div><div class="fg"><label>Reset solicitado</label><select id="resetFilterV130" class="fi" onchange="renderResetRequestsV130()"><option value="aberto">Abertos</option><option value="todos">Todos</option></select></div></div><div id="responsibleAccessResultsV130" class="search-results"></div><h4 style="margin:18px 0 8px">Solicitações recentes</h4><div id="resetRequestsV130">${reqs.length?reqs.map(r=>`<div class="student-row"><div><strong>${esc(r.alunoNome||'-')}</strong><div class="muted">${esc(r.matricula||'')} · ${esc(r.status||'aberto')} · ${humanDate(r.criadoEm)}</div></div><button class="btn btn-light" onclick="openResponsibleAccessManagerV130('${esc(r.alunoId)}')">Abrir</button></div>`).join(''):'<div class="empty-soft">Nenhuma solicitação recente.</div>'}</div></div></div>`;renderResponsibleAccessSearchV130()
}
function userRowHtmlV130(u){const cashRole=accessCashPermissionV164(u);return `<div class="user-row-v130"><div><strong>${esc(u.nome)}</strong><div class="tiny-v130">${esc(u.observacao||'')} · Caixa: ${esc(cashRole==='gestor'?'gestor':cashRole==='nenhum'?'sem acesso':'operador')}</div></div><div>${u.email?esc(u.email):'<span class="muted">E-mail não definido</span>'}<div class="tiny-v130">${u.authUid?'UID vinculado':'UID ainda não vinculado'}</div></div><div><span class="pill-v130 ${roleClassV130(u.perfil)}">${roleLabelV130(u.perfil)}</span></div><div>${u.ativo===false?'<span class="pill-v130 off">Inativo</span>':'<span class="badge b-green">Ativo</span>'}</div><div class="actions"><button class="btn btn-light" onclick="openUserProfileV130('${u.id}')">Editar</button></div></div>`}
async function openUserProfileV130(id=''){
  let u={id:'',nome:'',email:'',perfil:'secretaria',turnos:['manha'],caixaPermissao:'operador',ativo:true,permissoes:basePermissionsV130('secretaria'),observacao:''};if(id){const s=await db.collection(ACCESS_USERS_V130).doc(id).get();if(s.exists)u={id:s.id,...s.data()}}
  u.caixaPermissao=accessCashPermissionV164(u);
  openModal(id?'Editar perfil de acesso':'Novo perfil de acesso',`<div class="form-row"><div class="fg"><label>Identificador do usuário</label><input id="accIdV130" class="fi" value="${esc(u.id)}" ${id?'disabled':''} placeholder="ex: daniele"></div><div class="fg"><label>Nome</label><input id="accNameV130" class="fi" value="${esc(u.nome)}"></div></div><div class="fg"><label>E-mail de login</label><input id="accEmailV130" class="fi" type="email" value="${esc(u.email||'')}" placeholder="e-mail usado para entrar no sistema"></div><div class="form-row"><div class="fg"><label>Perfil</label><select id="accPerfilV130" class="fi" onchange="accProfileChangedV130()"><option value="admin" ${u.perfil==='admin'?'selected':''}>Administrador</option><option value="gestao" ${u.perfil==='gestao'?'selected':''}>Gestão</option><option value="secretaria" ${u.perfil==='secretaria'?'selected':''}>Secretaria</option><option value="cantina" ${u.perfil==='cantina'?'selected':''}>Cantina</option></select></div><div class="fg"><label>Turnos</label><select id="accTurnosV130" class="fi" multiple><option value="manha" ${(u.turnos||[]).includes('manha')?'selected':''}>Manhã</option><option value="tarde" ${(u.turnos||[]).includes('tarde')?'selected':''}>Tarde</option></select></div></div><div class="fg"><label>Acesso ao caixa</label><select id="accCashRoleV164" class="fi"><option value="nenhum" ${u.caixaPermissao==='nenhum'?'selected':''}>Sem acesso ao caixa</option><option value="operador" ${(u.caixaPermissao||'operador')==='operador'?'selected':''}>Operador — abre, assume, movimenta e justifica</option><option value="gestor" ${u.caixaPermissao==='gestor'?'selected':''}>Gestor — opera e decide divergências</option></select></div><div class="fg"><label>Observação</label><input id="accObsV130" class="fi" value="${esc(u.observacao||'')}"></div><label class="switch-line"><span><strong>Usuário ativo</strong><small class="muted">Usuários inativos não entram no sistema.</small></span><input id="accActiveV130" type="checkbox" ${u.ativo!==false?'checked':''}></label><div class="alert" style="margin-top:12px">Este cadastro define o perfil e as permissões do usuário no sistema.</div><button class="btn btn-primary" style="width:100%;margin-top:14px" onclick="saveUserProfileV130('${esc(id)}')">Salvar perfil</button>`)
}
function accProfileChangedV130(){}
async function saveUserProfileV130(existingId=''){
  const id=existingId||sanitizeKeyV120($('#accIdV130').value);if(!id)return appMessage('Informe um identificador para o usuário.');const perfil=$('#accPerfilV130').value,turnos=[...$('#accTurnosV130').selectedOptions].map(o=>o.value);const data={id,nome:$('#accNameV130').value.trim(),email:normalizeEmailV130($('#accEmailV130').value),perfil,turnos,caixaPermissao:$('#accCashRoleV164')?.value||defaultCashPermissionV164(perfil),ativo:$('#accActiveV130').checked,permissoes:basePermissionsV130(perfil),observacao:$('#accObsV130').value.trim()};if(!data.nome)return appMessage('Informe o nome.');
  try{const u=authObjV130()?.currentUser;if(!u)throw new Error('Entre novamente na Gestão.');const token=await u.getIdToken();const r=await fetch('/api/familias?modulo=security',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({acao:'save_staff_profile',...data})}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'Não foi possível salvar o perfil.');await audit('perfil_acesso_salvo',{usuarioAcessoId:id,perfil,email:data.email});closeModal();toast('Perfil salvo.');renderUsuariosAcessosV130()}catch(e){appMessage(e.message||'Não foi possível salvar o perfil.')}
}
...
async function requestParentResetV130(){const mat=$('#parentMatricula')?.value.trim();if(!mat)return appMessage('Informe a matrícula.');const candidates=state.students.filter(a=>a.matricula===mat);if(!candidates.length)return appMessage('Matrícula não encontrada.');const a=candidates[0];await db.collection(PARENT_RESET_REQUESTS_V130).add({alunoId:a.id,alunoNome:a.nome,matricula:a.matricula,turma:a.turma,status:'aberto',origem:'portal_responsavel',criadoEm:nowIso()});$('#parentMsg').innerHTML='<div class="alert warn" style="margin-top:12px"><strong>Solicitação registrada.</strong><br>Para proteger a conta do aluno, a nova senha precisa ser liberada pela secretaria. Entre em contato com a escola e solicite o link temporário de redefinição.</div>'}
...
async function saveParentPasswordV130(){const p1=$('#parentNewPassV130')?.value||'',p2=$('#parentNewPass2V130')?.value||'';if(p1!==p2)return appMessage('As senhas não conferem.');if(!passwordOkV130(p1))return appMessage('A senha precisa ter pelo menos 6 caracteres.');const a=state.parentStudent,salt=await randomTokenV130(12),hash=await hashParentPasswordV130(p1,salt);await db.collection(PARENT_ACCESS_V130).doc(a.id).set({alunoId:a.id,alunoNome:a.nome,matricula:a.matricula,turma:a.turma,ativo:true,salt,senhaHash:hash,senhaCriadaEm:nowIso(),tentativasInvalidas:0,bloqueadoAte:null,atualizadoEm:nowIso()},{merge:true});await audit('senha_responsavel_criada',{alunoId:a.id});closeModal();toast('Senha salva.');renderParentPortal()}

async function renderResetRequestsV130(){
  const host=$('#resetRequestsV130');if(!host)return;
  const filter=$('#resetFilterV130')?.value||'aberto';host.innerHTML='<div class="v151-inline-loader">Atualizando...</div>';
  try{
    const snap=await db.collection(PARENT_RESET_REQUESTS_V130).orderBy('criadoEm','desc').limit(50).get();
    const rows=snap.docs.map(d=>({id:d.id,...d.data()})).filter(r=>filter==='todos'||String(r.status||'aberto')==='aberto');
    host.innerHTML=rows.length?rows.map(r=>`<div class="student-row"><div><strong>${esc(r.alunoNome||'-')}</strong><div class="muted">${esc(r.matricula||'')} · ${esc(r.status||'aberto')} · ${humanDate(r.criadoEm)}</div></div><button class="btn btn-light" onclick="openResponsibleAccessManagerV130('${esc(r.alunoId)}')">Abrir</button></div>`).join(''):'<div class="empty-soft">Nenhuma solicitação neste filtro.</div>';
  }catch(e){host.innerHTML=`<div class="alert danger">${esc(e.message||'Não foi possível atualizar as solicitações.')}</div>`}
}
window.renderResetRequestsV130=renderResetRequestsV130;
...
async function clearParentPasswordV130(alunoId){if(!await appConfirm('A senha atual será removida e o responsável voltará ao primeiro acesso por validação.',{title:'Resetar senha do responsável?',confirmLabel:'Resetar senha',cancelLabel:'Voltar',danger:true}))return;await db.collection(PARENT_ACCESS_V130).doc(alunoId).set({ativo:false,senhaHash:null,salt:null,resetadoPorId:state.user.id,resetadoPorNome:state.user.nome,resetadoEm:nowIso(),atualizadoEm:nowIso()},{merge:true});await audit('senha_responsavel_resetada',{alunoId});closeModal();toast('Acesso resetado.')}
async function blockParentAccessV130(alunoId){if(!await appConfirm('O responsável não conseguirá entrar com a senha atual até que o acesso seja reativado.',{title:'Bloquear acesso?',confirmLabel:'Bloquear acesso',cancelLabel:'Voltar',danger:true}))return;await db.collection(PARENT_ACCESS_V130).doc(alunoId).set({ativo:false,bloqueadoPorId:state.user.id,bloqueadoPorNome:state.user.nome,bloqueadoEm:nowIso(),atualizadoEm:nowIso()},{merge:true});await audit('acesso_responsavel_bloqueado',{alunoId});closeModal();toast('Acesso bloqueado.')}
...
async function generateParentResetLinkV130(alunoId){
  ...
  if(!a)return appMessage('Aluno não encontrado.');
...
/* =========================================================
   ...
   Auditoria e correção definitiva do Meu Piaget
...
async function parentPasswordLoginV130(){
  ...
  if(!mat||!pass)return appMessage('Informe matrícula e senha.');
...
async function submitParentResetPasswordV130(){
  ...
  if(!ctx)return appMessage('Sessão de redefinição não encontrada. Abra novamente o link enviado pela secretaria.');
  if(p1!==p2)return appMessage('As senhas não conferem.');
  if(!passwordOkV130(p1))return appMessage('A senha precisa ter pelo menos 6 caracteres.');
...
/* V1.6.0-rc2.7.28 — logout canônico do botão superior. */
