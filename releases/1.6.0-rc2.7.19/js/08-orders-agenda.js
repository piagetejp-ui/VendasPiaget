...
(()=>{
'use strict';
const V150_VERSION='1.6.0-rc2.7.19';
...
async function loadOrdersForStudentV150(studentId){
  try{const q=await db.collection('pedidos').where('alunoId','==',studentId).limit(100).get();return q.docs.map(d=>({id:d.id,...d.data()}));}
  catch(e){const q=await db.collection('pedidos').limit(200).get().catch(()=>({docs:[]}));return q.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.alunoId===studentId)}
}
...
async function getAgendaDataV150(day){const [occSnap,stockSnap,allSnap]=await Promise.all([db.collection('ocorrencias_entrega').where('dataChave','==',day).limit(300).get().catch(()=>({docs:[]})),db.collection('disponibilidade_salgados').doc(day).get().catch(()=>({exists:false})),db.collection('ocorrencias_entrega').where('dataChave','<',dateKey()).limit(100).get().catch(()=>({docs:[]}))]);const rows=occSnap.docs.map(d=>({id:d.id,...d.data()}));const previous=allSnap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.dataChave<dateKey()&&!V150_FINAL_STATUSES.includes(x.status));return{rows,stock:stockSnap.exists?stockSnap.data():{},previous}}
...
/* atualização de versão herdada removida pela V1.6.0-rc2.7.19 */
})();
