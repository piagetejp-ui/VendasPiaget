...
window.financialAccountsSnapshotV167=financialAccountsSnapshotV167=async function financialAccountsSnapshotEfficientV219(force=false){
 ...
 const docs=snap.docs.map(d=>{const children=linkedStudentsLocalV219(d.id),stored=d.data()||{},preferred=children.find(a=>String(a.id)===String(stored.alunoPrincipalId||''))||children[0]||null,ids=children.length?children.map(a=>a.id):(Array.isArray(stored.alunosIds)?stored.alunosIds:[]),data={...stored,responsavelId:d.id,alunosIds:ids,alunoPrincipalId:preferred?.id||stored.alunoPrincipalId||ids[0]||null,quantidadeAlunosAtivos:children.length||Number(stored.quantidadeAlunosAtivos||ids.length||0)};return{id:data.alunoPrincipalId||d.id,ref:d.ref,exists:true,data:()=>data}});
