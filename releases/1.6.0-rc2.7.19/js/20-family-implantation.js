...
async function loadFamilyCoreV167(){
 ...
 try{configRead=await db.collection('configuracoes').doc('sistema').get()}catch(e){
   ...
   if(String(e?.code||'').includes('permission-denied')||String(e?.message||'').includes('Missing or insufficient permissions'))throw new Error('A sessão foi validada, mas o Firestore recusou a leitura inicial do Meu Piaget. Publique também as Firestore Rules da RC2.7.15 e tente novamente.');
   throw e;
 }
 const [cfg,ps,ss,ms,ds,cs,is]=await Promise.all([
   Promise.resolve(configRead),
   db.collection('produtos').limit(200).get().catch(()=>({docs:[]})),
   ...
   db.collection('catalogo_categorias').limit(100).get().catch(()=>({docs:[]})),
   db.collection('catalogo_itens').limit(300).get().catch(()=>({docs:[]}))
 ]);
