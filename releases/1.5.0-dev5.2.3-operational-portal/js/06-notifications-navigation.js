...
async function refreshNotificationBadgeV134(){
  const bell=$('#notifBellV134'),countEl=$('#notifCountV134');if(!bell||!countEl||!state.user)return;
  const rows=await loadNotificationsForUserV134(100),uid=state.user.id,open=rows.filter(n=>(n.status||'pendente')==='pendente'&&!(n.lidoPor||[]).includes(uid)).length;
...
function ensureNotificationBellV134(){
  if(!state.user)return;const actions=document.querySelector('.top-actions');if(!actions||$('#notifBellV134'))return;
...
function notificationHtmlV134(n,compact=false){const unread=!(n.lidoPor||[]).includes(state.user?.id),urgent=(n.prioridade==='urgente'||n.prioridade==='alta');return `<div class="notif-card-v134 ${unread?'unread':''} ${urgent?'urgent':''}"><div><div class="notif-title-v134">${esc(n.titulo||'Notificação')}</div><div class="notif-msg-v134">${esc(n.mensagem||'')}</div><div class="notif-meta-v134">${priorityBadgeV134(n.prioridade)} · ${esc(statusLabelV120?statusLabelV120(n.status||'pendente'):n.status||'pendente')} · ${humanDate(n.criadoEm)}${n.criadoPorNome?` · criado por ${esc(n.criadoPorNome)}`:''}</div></div><div class="notif-actions-v134">${notificationActionButtonV134(n)}${unread?`<button class="btn btn-outline" onclick="markNotificationReadV134('${n.id}')">Marcar lida</button>`:''}${(n.status||'pendente')==='pendente'?`<button class="btn btn-light" onclick="resolveNotificationV134('${n.id}')">Resolver</button>`:''}</div></div>`}
async function openNotificationsPanelV134(){const rows=(await loadNotificationsForUserV134(80)).slice(0,12);openModal('Notificações',`<div class="actions" style="margin-bottom:12px"><button class="btn btn-primary" onclick="closeModal();navigate('notificacoes')">Abrir central</button><button class="btn btn-outline" onclick="markAllNotificationsReadV134()">Marcar todas como lidas</button></div><div class="notif-wrap-v134">${rows.length?rows.map(n=>notificationHtmlV134(n,true)).join(''):'<div class="empty-soft">Nenhuma notificação para seu perfil.</div>'}</div>`)}
async function markNotificationReadV134(id){if(!state.user)return;await db.collection(NOTIFICATIONS_V134).doc(id).set({lidoPor:FIELD.arrayUnion(state.user.id),lidoEm:nowIso()},{merge:true});toast('Notificação marcada como lida.');refreshNotificationBadgeV134();if(state.currentPage==='notificacoes')renderNotificacoesV134();}
async function markAllNotificationsReadV134(){const rows=await loadNotificationsForUserV134(150),batch=db.batch();rows.forEach(n=>batch.set(db.collection(NOTIFICATIONS_V134).doc(n.id),{lidoPor:FIELD.arrayUnion(state.user.id),lidoEm:nowIso()},{merge:true}));await batch.commit();toast('Notificações marcadas como lidas.');closeModal();refreshNotificationBadgeV134();if(state.currentPage==='notificacoes')renderNotificacoesV134();}
async function resolveNotificationV134(id){const actor=currentActorV134();await db.collection(NOTIFICATIONS_V134).doc(id).set({status:'resolvida',resolvidoEm:nowIso(),resolvidoPorId:actor.id,resolvidoPorNome:actor.nome,lidoPor:FIELD.arrayUnion(actor.id)},{merge:true});await auditV134('notificacao_resolvida',{notificacaoId:id});toast('Notificação resolvida.');refreshNotificationBadgeV134();if(state.currentPage==='notificacoes')renderNotificacoesV134();}
