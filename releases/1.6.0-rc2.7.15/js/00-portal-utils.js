...
const DEFAULT_FAMILY_PUBLIC_ORIGIN_V178='https://meupiaget.com.br';
function familyPublicOriginV178(){
  const configured=String(window.PIAGET_FAMILY_PUBLIC_ORIGIN||state?.config?.urlMeuPiaget||DEFAULT_FAMILY_PUBLIC_ORIGIN_V178).trim();
  try{return new URL(configured,DEFAULT_FAMILY_PUBLIC_ORIGIN_V178).origin}catch(_){return DEFAULT_FAMILY_PUBLIC_ORIGIN_V178}
}
function familyPublicUrlV178(path=''){
  const base=familyPublicOriginV178();
  const clean=String(path||'').trim();
  if(!clean)return `${base}/`;
  try{return new URL(clean.startsWith('/')?clean:`/${clean}`,`${base}/`).toString()}catch(_){return `${base}/${clean.replace(/^\/+/, '')}`}
}
window.PIAGET_FAMILY_PUBLIC_ORIGIN=window.PIAGET_FAMILY_PUBLIC_ORIGIN||DEFAULT_FAMILY_PUBLIC_ORIGIN_V178;
window.familyPublicOriginV178=familyPublicOriginV178;
window.familyPublicUrlV178=familyPublicUrlV178;
