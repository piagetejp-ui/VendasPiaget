...
async function familyMovesV181(students){
  ...
  for(const a of students||[]){
    jobs.push(db.collection('movimentos_conta').where('alunoId','==',a.id).get().catch(()=>({docs:[]})));
    jobs.push(db.collection('movimentos_conta').where('alunosIds','array-contains',a.id).get().catch(()=>({docs:[]})));
  }
